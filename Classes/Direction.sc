...
// Direction: bar-local instructions such as tempo, rehearsal marks and text.
//
...
// Whole BPM and one notatable beat make a well-formed mark. Writer
// vocabularies remain writer checks.
//
...
Direction {
    ...
    *initClass {
    ...
    //
    // One fact said twice is refused, not resolved.
    ...
    *text { |text, offset = 0| ^this.of(\text, text, offset) }
    ...
    // A tempo with no words.
    // The unit leads because the mark reads `4 = 120`.
    //
    ...
    *metronome { |unit, perMinute, offset = 0, text, beat, bpm|
    ...
    // See Note [A tempo ramp is two directions].
    //
    // The start says the ramp and target.
    //
    // The stop says where it ends and which id it closes.
    //
    ...
    *tempoRampStop { |offset = 0, id = 1|
    ...
    // id held here.
    //
    // It needs explicit offsets because a bar has no attack position.
    //
    // Build both endpoints before attaching either.
    //
    // >>> Direction.tempoRamp([Measure("1/4", "c4"), Measure("1/4", "d4")], "rit.").last.directions.first.isRampStop
    // true
    *tempoRamp { |bars, text, startOffset = 0, stopOffset = 0, id = 1,
        ...
        group.do { |bar|
            if (bar.isKindOf(Measure).not) {
                Error(
                    "Direction: a tempo ramp expects Measures, got %.".format(bar)
                ).throw
            }
        };
        if (group.size == 1 and: { start.offset >= stop.offset }) {
            Error(
                "Direction: a one-bar tempo ramp starts at % and stops at %. The stop "
                "must be later.".format(start.offset, stop.offset)
            ).throw
        };
    ...
    *of { |kind, text, offset = 0, unit, perMinute, beat, bpm, edge, id|
        ...
        if (kinds.includes(checked).not) {
            Error(
                "Direction: \"%\" is not a direction. The kinds are %.".format(kind, kinds)
            ).throw
        };
        ...
        // Prose uses `Marking` rules.
        // Absence was checked just above.
    ...
    *checkedEdge { |kind, edge|
        if (kind != \tempoRamp) {
            if (edge.notNil) {
                Error(
                    "Direction: a % direction cannot carry edge %. Only tempoRamp uses "
                    "paired endpoints.".format(kind, edge)
                ).throw
            };
            ^nil
        };
        if (edge.isNil) {
            Error(
                "Direction: a tempo ramp endpoint needs edge start or stop. Use "
                "Direction.tempoRampStart or Direction.tempoRampStop."
            ).throw
        };
        if (edges.includes(edge.asSymbol).not) {
            Error(
                "Direction: \"%\" is not a tempo ramp edge. Use start or stop."
                .format(edge)
            ).throw
        };
        ^edge.asSymbol
    ...
    *checkedId { |kind, id|
        if (kind != \tempoRamp) {
            if (id.notNil) {
                Error(
                    "Direction: a % direction cannot carry id %. Only tempoRamp uses endpoint ids.".format(kind, id)
                ).throw
            };
            ^nil
        };
        if ((id ? 1).isKindOf(Integer).not or: { (id ? 1) < 1 }) {
            Error(
                "Direction: a tempo ramp id must be a positive integer, got %.".format(id)
            ).throw
        };
        ^id ? 1
    }

    // See Note [Two names for one beat].
    //
    // Either name, never both.
    *prOneOfTwoNames { |kept, alias, keptName, aliasName|
    ...
    // kind and at least one tempo fact.
    //
    // A ramp start takes the same fields.
    //
    // `hasMetronome` stays point-tempo only.
    *checkMetronome { |kind, text, unit, perMinute, edge|
        if (unit.isNil != perMinute.isNil) {
            Error(
                "Direction: a metronome mark needs both unit and perMinute. got % and %.".format(unit, perMinute)
            ).throw
        };
        if (this.prIsRampStop(kind, edge)) {
            if (text.notNil) {
                Error(
                    "Direction: a tempo ramp stop cannot carry text, got \"%\".".format(text)
                ).throw
            };
            if (unit.notNil) {
                Error(
                    "Direction: a tempo ramp stop cannot carry a metronome mark. Put "
                    "the target speed on the start."
                ).throw
            };
            ^this
        };
        if (unit.notNil and: { this.prStatesASpeed(kind).not }) {
            Error(
                "Direction: a % direction cannot carry a metronome mark. Use tempo, metronome or tempoRampStart.".format(kind)
            ).throw
        };
        if (this.prStatesASpeed(kind) and: { text.isNil } and: { unit.isNil }) {
            Error(
                "Direction: a % needs text, a metronome mark, or both.".format(kind)
            ).throw
        }
    }
    ...
    *checkedUnit { |unit|
        ...
        if (exact.notation.isNil) {
            Error(
                "Direction: metronome beat % is not writable as one note value.".format(exact)
            ).throw
        };
        ^exact
    }

    // Whole BPM is the shared count shape.
    *checkedPerMinute { |value|
        if (value.isKindOf(Integer).not) {
            Error(
                "Direction: perMinute must be a positive integer, got %."
    ...
    *checkedOffset { |offset|
        ...
        if (exact < Duration(0, 1)) {
            Error(
                "Direction: offset must be zero or later, got %.".format(exact)
            ).throw
        };
        ^exact
    }

    // Exact kind test; it must stay one.
    ...
    // their own number. A ramp target isn't one of those.
    //
    ...
    // At the bar's start. A direction sits there unless it says
    // otherwise. A writer never has to place that case specially.
    //
    // >>> Direction.text("Rit.").atBarStart                   -> true
    // >>> Direction.text("Rit.", Duration(1, 2)).atBarStart   -> false
    atBarStart { ^offset == Duration(0, 1) }

    // The same mark counted in quarter notes, nil when there is no mark.
    // Both callers want it in those units: MusicXML's `<sound tempo=...>` and
    ...
    // has its own rules about what a count may be.
    // `PlaybackTempoMap` admits a fractional one where a written mark
    // doesn't. Here so there is one copy of it.
    //
    // A plain number rather than a `Duration`: the exactness rule is
    // about *time*. 67.5 beats a minute is a rate, not a moment.
    //
    ...
    *quarterPerMinuteOf { |unit, perMinute|
    ...
    // Offset, mark and endpoint all participate in equality: two "Rit."s at
    // different points are two instructions. `4 = 60` isn't `4 =
    // 120`. A start isn't its stop.
    //
    ...
    // Each part only when there is one. The offset is labeled because
    // two optional trailing values would otherwise read as each
    // other. A non-default id is labeled for the same reason.
    printOn { |stream|
    ...
    // Store through the helper for this kind. `beat` and `bpm` are source
    // aliases; stored source uses `unit` and `perMinute`.
    //
    // >>> Direction.rehearsalMark("A").asCompileString
    // Direction.rehearsalMark("A")
    // >>> Direction.text("senza vibrato", Duration(1, 2)).asCompileString
    // Direction.text("senza vibrato", Duration(1, 2))
    storeOn { |stream|
        var head = if (kind == \tempoRamp) {
            "tempoRamp" ++ if (edge == \start) { "Start" } { "Stop" }
        } { kind.asString };
        var wrote = false;

        stream << "Direction." << head << "(";
        // Ramp stops have no text.
        if (head != "tempoRampStop") {
            stream << text.asCompileString;
            wrote = true
        };
        if (this.atBarStart.not) {
            if (wrote) { stream << ", " };
            stream <<< offset;
            wrote = true
        };
        if (id.notNil and: { id != 1 }) {
            if (wrote) { stream << ", " };
            stream << "id: " << id;
            wrote = true
        };
        unit !? {
            if (wrote) { stream << ", " };
            stream << "unit: " <<< unit << ", perMinute: " << perMinute
        };
        stream << ")"
    }
}
