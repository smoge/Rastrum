...
// and hooks.
//
// A beam may cross a barline inside one timeline but not a voice or
// staff.
//
// Preparation may split a beamed leaf. Endpoints follow the
// fragments.


// Validator: structural checks on a score tree before anything writes.
//
// Tree mistakes the model can hold and writers would catch unevenly.
// Reads and answers the tree it was given. No repair, no writer
// syntax. Some rules are repeated in writers when a writer depends on
// them.
//
...
Validator {
    ...
    //
    // >>> Validator.validate(RhythmTree.measure(Meter(4, 4), [1, 1, 1, 1])).isKindOf(Measure)   -> true
    // >>> try { Validator.validate(Measure(Meter(4, 4), [MusicNote(60, Duration(1, 2))])) } { \refused }   -> refused
    *validate { |element, requirePrepared = false|
    ...
    //
    // See Note [A tempo ramp is two directions] in Direction.sc.
    //
    ...
    *prCheckDirectionSpans { |element|
    ...
    //
    // Refuse any second open ramp. The model says nothing about
    // combining two.
    *prCheckDirectionSpanRun { |measures, ending|
            ...
            this.inSpanOrder(ramps).do { |endpoint|
                if (endpoint.isRampStop) {
                    if (open[endpoint.id].isNil) {
                        Error(
                            "Validator: tempo ramp stop id % has no open start in this staff."
                            .format(endpoint.id)
                        ).throw
                    };
                    ...
                    if (open.notEmpty) {
                        Error(
                            "Validator: % starts while another tempo ramp is open. "
                            "Close the first before starting another."
                            .format(this.prNameOf(endpoint))
                        ).throw
                    };
        ...
        if (open.notEmpty) {
            Error(
                "Validator: tempo ramp id % is still open when %. Add a stop in the same staff."
                .format(open.keys.asArray.sort.join(", "),ending)
            ).throw
        };
        ^this
    }
    ...
    *prCheckTuplet { |tuplet|
        ...
        if (ratio.numerator <= 0 or: { ratio.denominator <= 0 }) {
            Error(
                "Validator: % has a non-positive tuplet multiplier. The ratio must be greater than zero."
                .format(tuplet)
            ).throw
        };
        ...
        if (tuplet.actualNotes.isKindOf(Integer).not
            ...
            Error(
                "Validator: % prints invalid tuplet counts %:%. Counts must be positive integers."
                .format(tuplet, tuplet.actualNotes, tuplet.normalNotes)
            ).throw
        };
        ...
        if ((tuplet.normalNotes * ratio.denominator)
            ...
            Error(
                "Validator: % prints %:% but scales time by %. The printed ratio and multiplier disagree."
                .format(tuplet, tuplet.actualNotes, tuplet.normalNotes, ratio)
            ).throw
        };
    ...
    *prCheckVoicePlacement { |voice|
        if (voice.parent.isKindOf(Measure).not) {
            Error(
                "Validator: a Voice held by % is outside any Measure."
                .format(voice.parent ?? { "nothing" })
            ).throw
        };
        ^this
    }

    // The spine contents: a score holds staves
    //
    // A staff holds bars Placement checks ask what holds an element.
    // This asks what it holds.
    *prCheckContents { |container|
        if (container.isKindOf(MusicScore)) {
            container.children.do { |child|
                if (child.isKindOf(Staff).not) {
                    Error(
                        "Validator: a score may hold only staves, got a %."
                        .format(child.class)
                    ).throw
                }
            };
        ...
        if (container.isKindOf(Staff)) {
            container.children.do { |child|
                if (child.isKindOf(Measure).not) {
                    Error(
                        "Validator: a staff may hold only measures, got a %."
                        .format(child.class)
                    ).throw
                }
            };
        ...
        container.children.do { |child|
            if (child.isKindOf(MusicScore) or: { child.isKindOf(Staff) }
                ...
                Error(
                    "Validator: a % cannot hold a nested %."
                    .format(container.class, child.class)
                ).throw
            }
        };
        ^this
    ...
    *prCheckMeasurePlacement { |measure|
        ...
        if (holder.notNil and: { holder.isKindOf(Staff).not }) {
            Error(
                "Validator: a % bar must stand alone or belong to a Staff, got %."
                .format(measure.meter, holder.class)
            ).throw
        };
        ^this
    }
    ...
    *prCheckMeasure { |measure|
        ...
        if (measure.mixesVoicesWithElements) {
            Error(
                "Validator: a % bar mixes voices with loose elements. Use one timeline or voices, not both."
        ...
        measure.voices.do { |voice|
            if ((voice.duration == measure.barDuration).not) {
                Error(
                    "Validator: a % bar declares % but a voice holds %. Use "
                    "Measure.pickup or Measure.partial for a short bar."
                    .format(measure.meter, measure.barDuration, voice.duration)
                ).throw
            }
        };
    ...
    // A direction offset is local to its bar. It must be inside the
    // bar and on a boundary every voice shares.
    //
    ...
    *prCheckDirectionOffsets { |measure|
        ...
        measure.directions.do { |direction|
            if (direction.offset >= measure.barDuration) {
                Error(
                    "Validator: % is written % into a % bar that lasts %. Move an "
                    "offset at the span to the next bar."
                    .format(
            ...
            if (boundaries.any { |at| at == direction.offset }.not) {
                Error(
                    "Validator: % is written % into a % bar, but no leaf begins there "
                    "in every voice. Valid offsets are %."
                    .format(
                        this.prNameOf(direction),
                        direction.offset,
                        measure.meter,
                        boundaries.collect { |at| at.asString }.join(", ")
                    )
                ).throw
            };
        };
    ...
    // Endpoints in musical order: by offset, with stops before starts
    // at one offset. Public because `MusicXMLWriter` needs the same
    // order.
    //
    // See Note [A writer refuses what it depends on] in Writers/ScoreWriter.sc.
    *inSpanOrder { |endpoints|
    ...
    //
    // The kind is a symbol and a message is prose, so a ramp is
    // spelled out rather than interpolated: `\tempoRamp` would read
    // as `a tempoRamp start`. `ScoreWriter.prSubjectOf` answers the
    // same names for the writer layer.
    //
    // See Note [Where a direction may sit] in Writers/ScoreWriter.sc.
    *prNameOf { |direction|
    ...
    // Local offsets shared by every timeline in the bar. Normalize by
    // `metricOffset`, so pickups compare in bar-local terms.
    //
    // The walk is done once: asking inside the leaf loop rebuilt the
    // whole offset map per leaf per voice.
    *prLocalBoundariesIn { |measure|
        ...
        var perVoice = measure.voices.collect { |voice|
            voice.leaves.collect { |leaf|
                offsets[leaf] - measure.metricOffset
            }
        };
        ^perVoice.reduce { |a, b|
    ...
    *prCheckChord { |chord|
        if (chord.pitches.isEmpty) {
            Error(
                "Validator: a chord needs at least one pitch. Use a rest for silence."
            ).throw
        };
        ...
        chord.pitches.do { |pitch, index|
            if (index > 0 and: {
                ...
                Error(
                    "Validator: % appears twice in %. Use separate voices for unison doublings."
                    .format(pitch, chord)
                ).throw
            }
        };
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // Notes and chords, each spellable as one note head and nothing
    // else.
    //
    // A rest is silent, not ornamental. Grace leaves also carry no
    // ties, markings or spanners in this slice.


    // A whole-bar rest is bar silence, not a notehead value. It is
    // the one prepared leaf allowed to be non-notatable.
    *prCheckGraces { |leaf|
        ...
        leaf.graces.do { |grace|
            ...
            if (what.notNil) {
                Error(
                    "Validator: a grace group on % holds %."
                    .format(leaf.class, what)
                ).throw
            };
            if (grace.dur.isNotatable.not) {
                Error(
                    "Validator: grace duration % must be writable as one note head."
                    .format(grace.dur)
                ).throw
            };
            if (grace.hasGraces) {
    ...
    *prCheckNotatable { |leaf, wholeBarRest = false|
        ...
        if (leaf.dur.isNotatable.not) {
            Error(
                "Validator: % needs tie splitting before writing. Run ScorePrepare.run "
                "first."
                .format(leaf.dur)
            ).throw
        };
    ...
    *prCheckParents { |container|
        container.children.do { |child|
            if (child.parent !== container) {
                Error(
                    "Validator: % is held by % but has parent %."
                    .format(child, container, child.parent)
                ).throw
            }
        };
        ^this
    ...
    // Ties resolve within one voice. Measures are checked voice by
    // voice. A fragment with no measures is checked end to end.
    *prCheckTies { |element|
    ...
    *prCheckSpannerRun { |leaves, ending|
        ...
        leaves.do { |leaf, at|
            // Stops before starts on one leaf: a slur may end and
            // another may begin on the same note. That isn't the same
            // id reopening.
            leaf.spannerStops.do { |endpoint|
                var key = this.prSpannerKey(endpoint);
                if (open[key].isNil) {
                    Error(
                        "Validator: % stop id % on % has no open start."
                        .format(endpoint.kind, endpoint.id, leaf)
                    ).throw
                };
            ...
            leaf.spannerStarts.do { |endpoint|
                ...
                if (endpoint.permitsOverlap.not and: {
                    ...
                    Error(
                        "Validator: % starts a % while one is already open. Close the "
                        "first before starting another."
                        .format(leaf, endpoint.kind)
                    ).throw
                };
                if (open[key].notNil) {
                    Error(
                        "Validator: % id % is already open when % starts another. Use "
                        "different ids for overlapping spanners."
                        .format(endpoint.kind, endpoint.id, leaf)
                    ).throw
                };
                open[key] = leaf;
        ...
        if (open.notEmpty) {
            Error(
                "Validator: % left open when %. Add the stop in the same voice."
                .format(open.keys.asArray, ending)
            ).throw
        };
        ^this
    }
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // Other spanners cover a run. A glissando joins the next attack.
    // A tied continuation of the same note or chord is the same
    // attack.
    //
    // Chords pair by position. A note is one notehead for this check.
    // Unequal widths need a mapping the model doesn't store.

    // Both ends of one glissando, once the stop is reached.
    *prCheckGlissandoPair { |from, to, fromAt, toAt, leaves|
        var width = this.prGlissandoWidth(from);
        var between = 0, at = fromAt + 1;
        // `while` because `(a..b)` descends when the ends cross.
        while { at < toAt } {
            if (this.prContinues(leaves[at - 1], leaves[at]).not) {
                between = between + 1
            };
            at = at + 1;
        };
        if (between > 0) {
            Error(
                "Validator: glissando from % to % has % between them. It must join adjacent attacks."
                .format(
                    from,
                    to,
                    if (between == 1) { "1 attack" }
                    { between.asString ++ " attacks" }
                )
            ).throw
        };
        if (width != this.prGlissandoWidth(to)) {
            Error(
                "Validator: a glissando runs from % noteheads to %. Both ends need the "
                "same count. Writers pair by position."
                .format(width, this.prGlissandoWidth(to))
            ).throw
        };
        ^this
    }

    // A full tie into the same note or chord continues the attack.
    // Anything partial or changed is new.
    *prContinues { |before, leaf|
        if (before.isNil or: { leaf.isNil }) { ^false };
        if (before.isKindOf(Chord) and: { leaf.isKindOf(Chord) }) {
            ^(before.pitches == leaf.pitches)
                and: { before.tiedPitches.size == before.pitches.size }
        };
        if (before.isKindOf(MusicNote) and: { leaf.isKindOf(MusicNote) }) {
            ^before.tiesToNext and: { before.pitch == leaf.pitch }
        };
        ^false
    }
    ...
    *prGlissandoWidth { |leaf|
    ...
    // A rest cannot hold a pitch line. Run before generic spanner
    // pairing.
            ...
            if (node.isLeaf) {
                node.spanners.do { |endpoint|
                    if (endpoint.isGlissando and: { node.isKindOf(MusicRest) }) {
                        Error(
                            "Validator: % carries a glissando %, but a rest has no "
                            "pitch."
                            .format(node, endpoint.edge)
                        ).throw
                    }
                }
            }
    ...
    *prCheckBeamEndpoints { |leaf|
        ...
        if (leaf.isKindOf(MusicRest)) {
            Error(
                "Validator: % carries a beam endpoint, but rests cannot be beamed."
                .format(leaf)
            ).throw
        };
        ...
        if (beams.any { |endpoint| endpoint.isStart }
            ...
            Error(
                "Validator: % both stops and starts a beam. A note belongs to one beam "
                "group."
                .format(leaf)
            ).throw
        };
    ...
    *prCheckBeamRun { |leaves, ending|
        ...
        leaves.do { |leaf|
            ...
            stops.do { |endpoint|
                if (open.isNil) {
                    Error(
                        "Validator: beam stop id % on % has no open start in this timeline."
                        .format(endpoint.id, leaf)
                    ).throw
                };
                if (open != endpoint.id) {
                    Error(
                        "Validator: beam stop id % on % closes open beam id %."
                        .format(endpoint.id, leaf, open)
                    ).throw
                };
                open = nil;
            };
            starts.do { |endpoint|
                if (open.notNil) {
                    Error(
                        "Validator: % starts a beam while another is open. Close the first before starting another."
                        .format(leaf)
                    ).throw
                };
                open = endpoint.id;
        ...
        if (open.notNil) {
            Error(
                "Validator: beam id % is still open when %. Add the stop in the same timeline."
                .format(open, ending)
            ).throw
        };
        ^this
    ...
    *prCheckBeamable { |leaf|
        if (leaf.isKindOf(MusicRest)) {
            Error(
                "Validator: a beam runs through %, which is a rest. End the beam before it."
                .format(leaf)
            ).throw
        };
        if (leaf.dur < Duration.quarter) { ^this };
        Error(
            "Validator: a beam runs through %, which is written as %. Beamed notes must "
            "be shorter than a quarter."
            .format(leaf, leaf.dur)
        ).throw
    }
    ...
    *prStaffAbove { |node|
    ...
    // Within a staff, each voice position is one tie run across
    // barlines
    ...
    *prCheckTieRun { |leaves, ending|
        ...
        leaves.do { |leaf|
            ...
            pending.do { |pitch|
                if (arriving.any { |p| p == pitch }.not) {
                    Error(
                        "Validator: % ties to the next leaf, but % does not carry that pitch."
                        .format(pitch, leaf)
                    ).throw
                }
            };
        ...
        if (pending.notEmpty) {
            Error(
                "Validator: % ties onward, but %. Add the tied note in the same voice."
                .format(pending, ending)
            ).throw
        };
        ^this
    }
