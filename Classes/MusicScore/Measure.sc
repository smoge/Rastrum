// Measure: one bar, full or partial.
//
// Usually it fills its meter, but not always: a pickup is short and sits at the
// end of its notional bar, while a final incomplete bar is short and sits at
// the start. Two exact facts distinguish those: `barDuration` is the written
// span and `metricOffset` is where that span begins inside the nominal meter.
Measure : ScoreContainer {
    var <>meter, <barDuration, <metricOffset;

    // What is said over this bar: tempo, rehearsal mark or instruction. Stored
    // in attachment order and replaced by the setter.
    var <directions;

    // The clef this bar changes to, or nil for "carry on with the one in force".
    // Bar-local because clef changes happen inside a staff. Not a `Direction`:
    // a clef changes how later notes are read.
    var <clef;
    ...
    *pickup { |meter, children, barDuration|
    ...
    // A bar of silence: one rest lasting the whole meter. This is the shape
    // `wholeBarRests` recognizes.
    //
    ...
    *proportions { |meter, proportions, pitches|
    ...
    // The bar written out.
    //
    // `ScoreNotation` owns the grammar. Measure checks the line fills the bar.
    *notation { |meter, text|
        var written;
        ...
        if (written.notNil and: { written[2] ? false } and: {
            written[1].isEmpty }) {
            Error(
                "Measure.notation: \"%\" marks a pickup, but the bar was passed "
                "separately as \"%\". Use one String such as \"% %\"."
                .format(meter, text, meter, text)
            ).throw
        };
        if (written.notNil and: { written[1].notEmpty }) {
            Error(
                "Measure.notation: \"%\" already includes a bar. Don't also pass "
                "\"%\". Use one String or two arguments."
                .format(meter, text)
            ).throw
        };
    ...
    //
    // Negative durations are rests. Pitches cycle. Durations use the exact
    // duration coercion.
    //
    ...
    //
    // >>> Measure.durations(Meter(4, 4), [Duration(1, 2), Duration(-1, 2)]).leaves[1].class   -> MusicRest
    *durations { |meter, durations, pitches|
        ...
        if (exact.isEmpty) {
            Error(
                "Measure.durations: duration list cannot be empty. Use Measure.rest for a silent bar."
            ).throw
        };
        if (total != bar.duration) {
            Error(
                "Measure.durations: durations total %, but a % bar holds %. Use "
                "Measure.partial or Measure.pickup for a short bar."
                .format(total, bar, bar.duration)
            ).throw
        };
    ...
    initMeasure { |argMeter, argBarDuration, argMetricOffset|
        ...
        if (barDuration <= Duration(0, 1)) {
            Error(
                "Measure: bar duration must be positive, got %.".format(barDuration)
            ).throw
        };
        if (metricOffset < Duration(0, 1)) {
            Error(
                "Measure: metric offset must be zero or later, got %.".format(metricOffset)
            ).throw
        };
        if ((metricOffset + barDuration) > meter.duration) {
            Error(
                "Measure: duration % at offset % runs past the end of %.".format(barDuration, metricOffset, meter)
            ).throw
        };
        ^this
    }

    // A bar that fills its meter from the barline is the ordinary case. Nothing
    // downstream needs to treat it specially.
    //
    ...
    //
    // >>> Measure.pickup(Meter(4, 4), [MusicNote(60, Duration(1, 4))], Duration(1, 4)).isAnacrusis   -> true
    // >>> Measure.partial(Meter(4, 4), [MusicNote(60, Duration(1, 4))], Duration(1, 4)).isAnacrusis  -> false
    isAnacrusis {
    ...
    //
    // >>> Measure.partial(Meter(4, 4), [MusicNote(60, Duration(1, 4))], Duration(1, 4)).sitsAtBarline   -> true
    ...
    attach { |direction|
        if (direction.isKindOf(Direction).not) {
            Error(
                "Measure: % is not a Direction. Markings belong on leaves.".format(direction)
            ).throw
        };
    ...
    // Direction helpers mirror leaf marking helpers and answer this bar.
    // `offset` is how far into this bar the direction stands. `.tempo` may
    // include a metronome mark. `.metronome` is the wordless form.
    ...
    // Tempo-ramp endpoints, one bar at a time. `Direction.tempoRamp` is the
    // ordinary paired form.
    ...
    // Whole-bar rests: one plain rest, alone in its timeline, lasting the bar.
    //
    // Writers use this tree fact to choose silent-bar notation.
    //
    // Partial bars have none, the shape means a whole silent measure. Attached
    // rests have none. The silent-bar shape can't carry those facts.
    wholeBarRests {
        if (this.isPartial) { ^[] };
    ...
    // Voices run alongside each other. The bar lasts as long as the longest.
    duration {
        ^if (this.hasVoices) {
    ...
    // them and not the meter.
    //
    // A partial bar is full when it holds what it says it does.
    //
    ...
    // Full bars use `new`. Short bars name their helper.
    //
    // >>> Measure(Meter(2, 4), [MN("c2")]).asCompileString
    // Measure(Meter(2, 4), [MusicNote(MusicPitch("c[4]"), Duration(1, 2))])
    storeArgs { ^[meter, children.asArray] }

    // A bar puts the meter before the child list.
    prStoreHead {
        var selector = case
            { this.isPartial.not } { "Measure("         }
            { this.isAnacrusis   } { "Measure.pickup("  }
            { true               } { "Measure.partial(" };
        ^selector ++ meter.asCompileString ++ ", "
    }

    prStoreTail {
        if (this.isPartial.not) { ^")" };
        if (this.isAnacrusis) { ^", " ++ barDuration.asCompileString ++ ")" };
        ^", " ++ barDuration.asCompileString ++ ", "
            ++ metricOffset.asCompileString ++ ")"
    }

    // Directions and clef store through setters.
    prStoreSuffixes {
        var out = [];
        if (directions.notEmpty) { out = out.add([\directions_, directions, ""]) };
        clef !? { out = out.add([\clef_, nil, this.prStoreValue(clef)]) };
        ^out
    }
}
