// Independent timeline inside a bar.
//
// Two voices in one measure both start at the barline and both last
// the whole bar.
//
...
Voice : ScoreContainer {
    ...
    // A timeline is a run of leaves, so `Voice("g2 a2", "upper")` is
    // that run written out
    //
    ...
    *new { |children, name|
    ...
    // Store children as an Array, not the internal List.
    //
    // >>> Voice([MN("c4")]).asCompileString
    // Voice([MusicNote(MusicPitch("c[4]"), Duration(1, 4))])
    storeArgs { ^[children.asArray, name] }
}
