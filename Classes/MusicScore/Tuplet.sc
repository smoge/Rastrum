...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
// `ratio` is the arithmetic multiplier and reduces.
//
// `actualNotes` and `normalNotes` are the printed bracket counts and don't.
//
// The two describe one bracket, so they must agree. A multiplier-only tuplet
// has no authored pair, so counts are reduced.
//
// All three fields are read-only. Changing one alone would split the bracket.
...
//
// `actualNotes` and `normalNotes` keep the authored pair. The note above says
// what follows from that.
Tuplet : ScoreContainer {
    var <ratio, <actualNotes, <normalNotes;

    // The children slot takes a written run.
    //
    // See Note [A run of leaves is a run of leaves] in ScoreNotation.sc.
    //
    // So `Tuplet.ratio("3:2", "c4 d4 e4")` is the three leaves spelled out.
    //
    ...
    *prParseRatio { |string|
        ...
        if (parts.size != 2 or: { wrong }) {
            Error(
                "Tuplet: \"%\" is not a tuplet ratio. Use actual:normal, e.g. \"5:4\"."
                .format(string)
            ).throw
        };
    ...
    *prCheckCount { |value, what|
        if (value.isKindOf(Integer).not or: { value < 1 }) {
            Error(
                "Tuplet: % count must be a positive integer, got %."
                .format(what, value)
            ).throw
        };
        ^value
    ...
    initTuplet { |argRatio, argActual, argNormal|
        ...
        if (argRatio.isKindOf(String) or: { argRatio.isKindOf(Symbol) }) {
            if (argRatio.asString.contains(":")) {
                Error(
                    "Tuplet: \"%\" is a printed ratio, not a multiplier. Use Tuplet.ratio(\"5:4\", children)."
                    .format(argRatio)
                ).throw
            }
        };
        ...
        if ((normalNotes * ratio.denominator) != (actualNotes * ratio.numerator)) {
            Error(
                "Tuplet: printed ratio %:% does not match multiplier %. Use matching "
                "counts or Tuplet.multiplier."
                .format(actualNotes, normalNotes, ratio)
            ).throw
        };
        ^this
    }
    ...
    // Count pair is the stored form. Constructor choice is not a model fact.
    //
    // >>> Tuplet.multiplier(Duration(2, 3), [MN("c4")]).asCompileString
    // Tuplet.ratio(3, 2, [MusicNote(MusicPitch("c[4]"), Duration(1, 4))])
    prStoreHead {
        ^"Tuplet.ratio(" ++ actualNotes ++ ", " ++ normalNotes ++ ", "
    }
}
