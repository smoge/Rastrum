...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
// Hairpins, text, beams and glissandi don't overlap.
//
// Slurs may.
//
// Backends that can't spell a specific overlap refuse it. Each
...
//
// A spanner is a pair. The model stores endpoints.
//
// `Validator` checks that they meet. Slurs, hairpins, text spanners,
// beams and glissandi share this endpoint shape. Kind, edge and
// hairpin direction are closed sets.
//
// Starts carry the fact. Stops carry only the id.
//
// Hairpin starts carry a direction. Text starts carry prose and
// placement. Beam starts carry neither. `id` pairs the ends.
//
// Only slurs can overlap, so only `Spanner.slur` exposes it on the
// group helper.
Spanner {
    ...
    *initClass {
        ...
        // See Note [Kinds that cannot overlap].
        nonOverlapping = [\hairpin, \text, \beam, \glissando];
        // See Note [A head is a spelling, a direction is the fact].
        directionHeads = [
            ["crescendo",  \crescendo],
            ["cresc",      \crescendo],
    ...
    // the model words. They don't add names.
    ...
    *directionNamed { |head|
    ...
    // All written heads for a refusal that lists what this grammar knows.
    //
    ...
    *glissandoSpellings { ^["gliss", "glissando"] }
    ...
    // Compare by value: `includes` tests Strings by identity.
    //
    ...
    *text { |leaves, text, placement = \above|
    ...
    // attacks, so three attacks are two lines and the middle one
    // carries a stop and a start. Tie continuations aren't new
    // attacks. Syntax and writers don't remember which spelling
    // built them. Manual endpoints still serve pairs this helper will
    // not build, including one across a barline.

    // Pairwise over the leaves inside, brackets included. Answers the
    // children as written, so for example a tuplet stays a tuplet.
    //
    ...
    *glissando { |leaves|
        ...
        if (flat.size < 2) {
            Error(
                "Spanner.glissando: a glissando needs at least two attacks, got %."
                .format(flat.size)
            ).throw
        };
        ...
        flat.do { |leaf|
            if (leaf.isKindOf(MusicRest)) {
                Error(
                    "Spanner.glissando: a glissando holds a rest. Close the chain before it."
                ).throw
            }
        };
    ...
    *prGroup { |leaves, start, stop, label|
        ...
        if (group.size < 2) {
            Error(
                "Spanner: a % needs at least two leaves, got %."
                .format(start.kind, group.size)
            ).throw
        };
        [[group.first, "start"], [group.last, "stop"]].do { |end|
            if (end[0].isKindOf(ScoreLeaf).not) {
                Error(
                    "%: a % takes a leaf at its %, got %. A container may stand between the ends but not at one."
                    .format(label, start.kind, end[1], end[0])
                ).throw
            }
        };
    ...
    *prLeafGroup { |leaves, label, kind = \spanner|
        ...
        if (leaves.isKindOf(ScoreElement) and: { leaves.isLeaf.not }) {
            Error(
                "%: a % is not a run of leaves. Use a ScoreSelection and a filter, for example. ScoreSelection(bar).runs.first."
                .format(label, leaves.class)
            ).throw
        };
        ...
        group.do { |child|
            if (child.isKindOf(ScoreLeaf).not and: {
                ...
                Error(
                    "%: a % group holds leaves and brackets, got %."
                    .format(label, kind, child)
                ).throw
            }
        };
        ^group
    ...
    *prCheckText { |kind, edge, text, placement|
        ...
        if (text.notNil) {
            Error(
                "Spanner: a % % cannot carry text, got \"%\"."
                .format(kind, edge, text)
            ).throw
        };
        if (placement.notNil) {
            Error(
                "Spanner: a % % cannot carry placement %."
                .format(kind, edge, placement)
            ).throw
        };
        ^[nil, nil]
    }
    ...
    // says nothing about which direction it closed. A slur has none.
    // One place to write it means no pair can disagree.
    *prCheckDirection { |kind, edge, direction|
        if (kind == \hairpin and: { edge == \start }) {
            if (direction.isNil) {
                Error(
                    "Spanner: a hairpin start needs a direction. Use one of %."
                    .format(directions)
                ).throw
            };
        ...
        if (direction.notNil) {
            Error(
                "Spanner: a % % cannot carry direction %."
                .format(kind, edge, direction)
            ).throw
        };
        ^nil
    }
    ...
    *prCheck { |value, vocabulary, what|
        if (value.isNil) {
            Error(
                "Spanner: a spanner needs a %. Use one of %."
                .format(what, vocabulary)
            ).throw
        };
        if (vocabulary.includes(value.asSymbol).not) {
            Error(
                "Spanner: \"%\" is not a spanner %. Use one of %."
                .format(value, what, vocabulary)
            ).throw
        };
        ^value.asSymbol
    }
    ...
    *prCheckId { |id|
    ...
    // Source uses endpoint helpers. Starts carry direction/text when needed.
    //
    // >>> Spanner.slurStart(2).asCompileString   -> Spanner.slurStart(2)
    // >>> Spanner.textStart("sul pont.", 3, \below).asCompileString
    // Spanner.textStart("sul pont.", 3, \below)
    storeOn { |stream|
        stream << "Spanner." << kind << if (edge == \start) { "Start(" } { "Stop(" };
        if (edge == \start) {
            if (kind == \hairpin) { stream << "\\" << direction << ", " };
            if (kind == \text) { stream << text.asCompileString << ", " }
        };
        stream << id;
        if (edge == \start and: { kind == \text }) { stream << ", \\" << placement };
        stream << ")"
    }
}
