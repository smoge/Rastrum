...
// `ScorePrepare` asks through this API. Level 0 is the barline.
...
// than anything counted can begin a rest, but can't justify a
// crossing.

// Barlow's 1978 indispensability ranks pulses by metric importance.
// No two pulses tie. `stratification` chooses the ordered
// prime-product shape the formula reads.
//
// It refines `levelOf` but asks about attack thinning, not notation
// splitting. No writer reads this. Grouped meters refuse
// `stratification` because the formula takes products of primes, not
// additive groups.
...
Meter {
    ...
    *new { |count = 4, unit = 4|
        if (count.isKindOf(String) or: { count.isKindOf(Symbol) }) {
            // The default unit is harmless. Any other unit is a
            // second answer to one question.
            if (unit != 4) {
                Error(
                    "Meter: \"%\" already includes the unit, do not also pass %."
                    .format(count, unit)
                ).throw
            };
            ^this.prParse(count)
    ...
    // A Meter or printed spelling. `nil` is left alone for callers with their
    // own default or error. Numbers are refused here so `Measure(4, ...)` doesn't
    ...
    *asMeter { |value|
        ...
        Error(
            "Meter: % is not a meter. Use a Meter or a time signature such as \"4/4\" or \"5/8[2+3]\"."
            .format(value)
        ).throw
    }
    ...
    *grouped { |count, unit, groups|
    ...
    // The weakest line the meter counts: where one beat parts from
    // the next.
    //
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // A note may tie across a metric line. A rest can't hide a
    // counted beat line. Inside one beat, a dotted rest may stand
    // whole.
    ...
    // Barlow's indispensability values in pulse order.
    //
    ...
    stratification { |pulse|
        ...
        if (this.isGrouped) {
            Error(
                "Meter: indispensability is not defined for grouped meter %.".format(this)
            ).throw
        };
        perUnit = this.unitDuration / each;
        if (perUnit.denominator != 1 or: { perUnit.numerator < 1 }) {
            Error(
                "Meter: pulse % must be the unit of % or a division of it.".format(each, this)
            ).throw
        };
        ^Meter.prPrimeFactors(count) ++ Meter.prPrimeFactors(perUnit.numerator)
    }

    // The printed pair and grouping define equality and hashing.
    //
    ...
    // Use source forms, not slash summaries. Grouped meters name `grouped`.
    // Plain 4/4 stays explicit.
    //
    // >>> Meter(4, 4).asCompileString                 -> Meter(4, 4)
    // >>> Meter.grouped(5, 8, [2, 3]).asCompileString
    // Meter.grouped(5, 8, [2, 3])
    storeOn { |stream|
        if (this.isGrouped) {
            stream << "Meter.grouped(" << count << ", " << unit << ", " <<< groups << ")"
        } {
            stream << "Meter(" << count << ", " << unit << ")"
        }
    }

    // Digits, one slash and optional groups joined by `+`.
    // Strict so `asInteger` can't accept prefixes.
    *prParse { |text|
    ...
    *prRefuseSpelling { |text|
        Error(
            "Meter: \"%\" is not a time signature. Use count/unit, optionally with "
            "groups such as \"5/8[2+3]\".".format(text)
        ).throw
    }
    ...
    *prWhole { |value, what|
        if (value.isKindOf(Integer).not or: { value < 1 }) {
            Error(
                "Meter: % must be a positive integer, got %.".format(what, value)
            ).throw
        };
        ^value
    }
    ...
    *prCheckGroups { |groups, count|
        ...
        if (groups.isKindOf(String) or: { groups.isSequenceableCollection.not }) {
            Error(
                "Meter: grouping must be a list of whole units, got %.".format(groups)
            ).throw
        };
        if (groups.isEmpty) {
            Error(
                "Meter: grouping cannot be empty. Omit it for an undivided bar."
            ).throw
        };
        ...
        if (sum != count) {
            Error(
                "Meter: grouping % adds to %, but the meter has % units."
                .format(groups, sum, count)
            ).throw
        };
        ^groups.asArray.copy
    }
    ...
    *prW { |x| ^if (x == 0) { 0 } { 1 } }
    ...
    //
    // Generate a superset then filter by `levelOf`, so the level rule
    // has one definition.
