...
ScoreJSONReader {
    ...
    *checkEnvelope { |dict|
        ...
        if (dict.isKindOf(Dictionary).not) {
            Error(
                "ScoreJSONReader: expected a JSON object, got %"
                .format(dict.class)
            ).throw
        };
        format = dict["format"];
        ...
        if (format.asString != ScoreJSONContract.schemaFormat) {
            Error(
                "ScoreJSONReader: format is \"%\", expected \"%\""
                .format(format, ScoreJSONContract.schemaFormat)
            ).throw
        };
        if (version.isNil) {
            Error(
                "ScoreJSONReader: no \"version\" key in a % document"
                .format(ScoreJSONContract.schemaFormat)
            ).throw
        };
        // Exact string equality, as for integer fields.
        if (version.asString != ScoreJSONContract.schemaVersion.asString) {
            Error(
                "ScoreJSONReader: this reader understands % version %, but the document "
                "declares version %"
                .format(
                    ScoreJSONContract.schemaFormat,
                    ScoreJSONContract.schemaVersion,
                    version
                )
            ).throw
        };
    ...
    *keyHomes { |key|
    ...
    //
    // >>> ScoreJSONReader.recordKeys.detect { |row| row[0] == "direction" }[2].any { |key| key == "edge" }
    // true
    *recordKeys {
        ^[
    ...
    *prCheckKeys { |dict, what, allowed, required, named|
        required.do { |key|
            if (dict[key].isNil) {
                Error(
                    "ScoreJSONReader: % is missing required key \"%\". Required keys "
                    "are %."
                    .format(what, key, required)
                ).throw
            }
        };
        dict.keys.do { |key|
    ...
    *element { |dict|
        ...
        ^switch(kind,
            ...
            "measure", {
                ...
                if (dict["barDuration"].isNil != dict["metricOffset"].isNil) {
                    Error(
                        "ScoreJSONReader: a partial measure needs both \"barDuration\" "
                        "and \"metricOffset\"."
                    ).throw
                };
    ...
    // A meter is a pair with optional grouping the pair cannot carry.
    //
    ...
    *meter { |dict|
        ...
        if (raw.isKindOf(String) or: { raw.isSequenceableCollection.not }) {
            Error(
                "ScoreJSONReader: \"meterGrouping\" must be a list of whole units, got "
                "%"
                .format(raw)
            ).throw
        };
    ...
    *duration { |pair|
        if (pair.isKindOf(String) or: { pair.isSequenceableCollection.not }
            or: { pair.size != 2 }) {
            Error(
                "ScoreJSONReader: a duration must be [numerator, denominator], got %"
                .format(pair)
            ).throw
        };
        ^Duration(
    ...
    *integerValue { |value, what|
        ...
        if (parsed.asString != text) {
            Error(
                "ScoreJSONReader: % must be a whole number, got \"%\""
                .format(what, text)
            ).throw
        };
        ^parsed
    ...
    *floatValue { |value, what|
        ...
        while { index < text.size } {
            ...
            case
                ...
                { true } {
                    Error(
                        "ScoreJSONReader: % must be a plain decimal number, got \"%\""
                        .format(what, text)
                    ).throw
                };
            index = index + 1;
        ...
        if (seenDigit.not) {
            Error(
                "ScoreJSONReader: % has no digits, got \"%\""
                .format(what, text)
            ).throw
        };
        ^text.asFloat
    ...
    *positiveInteger { |value, what|
        ...
        if (parsed < 1) {
            Error(
                "ScoreJSONReader: % must be greater than zero, got %"
                .format(what, parsed)
            ).throw
        };
        ^parsed
    ...
    *withGraces { |leaf, list, style|
        ...
        if (list.isKindOf(String) or: { list.isSequenceableCollection.not }) {
            Error(
                "ScoreJSONReader: \"graces\" must be a list, got %"
                .format(list)
            ).throw
        };
        if (list.isEmpty) {
            Error(
                "ScoreJSONReader: \"graces\" cannot be empty. Omit the field for no "
                "grace group."
            ).throw
        };
    ...
    *graceDuration { |pair|
        ...
        if (dur.isNotatable.not) {
            Error(
                "ScoreJSONReader: grace duration % is not writable as one note head."
                .format(pair)
            ).throw
        };
        ^dur
    ...
    *graceLeaf { |dict|
        if (dict.isKindOf(Dictionary).not) {
            Error(
                "ScoreJSONReader: a grace must be an object with a \"type\", got %"
                .format(dict)
            ).throw
        };
        ...
        ^switch(dict["type"],
            ...
            {
                Error(
                    "ScoreJSONReader: a grace group cannot hold \"%\". Use "
                    "\"graceNote\" or \"graceChord\"."
                    .format(dict["type"])
                ).throw
            })
    }
    ...
    *withSpanners { |leaf, list|
        ...
        if (list.isKindOf(String) or: { list.isSequenceableCollection.not }) {
            Error(
                "ScoreJSONReader: \"spanners\" must be a list, got %"
                .format(list)
            ).throw
        };
        list.do { |entry|
            if (entry.isKindOf(Dictionary).not) {
                Error(
                    "ScoreJSONReader: a spanner must be an object with \"type\", "
                    "\"edge\" and \"id\", got %"
                    .format(entry)
                ).throw
            };
            ...
            if (entry["type"].isNil or: { entry["edge"].isNil }
                or: { entry["id"].isNil }) {
                Error(
                    "ScoreJSONReader: a spanner needs \"type\", \"edge\" and \"id\". "
                    "Got %"
                    .format(entry)
                ).throw
            };
            ...
            if (entry["text"].notNil and: { entry["text"].isKindOf(String).not }) {
                Error(
                    "ScoreJSONReader: a spanner's text must be a string, got %"
                    .format(entry["text"])
                ).throw
            };
    ...
    *withMarkings { |leaf, list|
        ...
        if (list.isKindOf(String) or: { list.isSequenceableCollection.not }) {
            Error(
                "ScoreJSONReader: \"markings\" must be a list, got %"
                .format(list)
            ).throw
        };
        list.do { |entry|
            if (entry.isKindOf(Dictionary).not) {
                Error(
                    "ScoreJSONReader: a marking must be an object with \"type\" and "
                    "\"value\", got %"
                    .format(entry)
                ).throw
            };
            if (entry["type"].isNil or: { entry["value"].isNil }) {
                Error(
                    "ScoreJSONReader: a marking needs both \"type\" and \"value\". Got "
                    "%"
                    .format(entry)
                ).throw
            };
    ...
    *withDirections { |measure, list|
        ...
        if (list.isKindOf(String) or: { list.isSequenceableCollection.not }) {
            Error(
                "ScoreJSONReader: \"directions\" must be a list, got %"
                .format(list)
            ).throw
        };
        list.do { |entry|
            if (entry.isKindOf(Dictionary).not) {
                Error(
                    "ScoreJSONReader: a direction must be an object, got %"
                    .format(entry)
                ).throw
            };
            ...
            if (entry["type"].isNil) {
                Error(
                    "ScoreJSONReader: a direction needs a \"type\". Got %"
                    .format(entry)
                ).throw
            };
            ...
            if (entry["text"].isNil and: { entry["unit"].isNil }
                ...
                Error(
                    "ScoreJSONReader: a direction needs \"text\" or a metronome mark. "
                    "Got %"
                    .format(entry)
                ).throw
            };
            if (entry["text"].notNil and: { entry["text"].isKindOf(String).not }) {
                Error(
                    "ScoreJSONReader: a direction's text must be a string, got %"
                    .format(entry["text"])
                ).throw
            };
            if (entry["unit"].isNil != entry["perMinute"].isNil) {
                Error(
                    "ScoreJSONReader: a metronome mark needs both \"unit\" and "
                    "\"perMinute\". Got %"
                    .format(entry)
                ).throw
            };
    ...
    *prCheckEndpoint { |entry|
        ...
        if (ramp.not) {
            ["edge", "id"].do { |key|
                if (entry[key].notNil) {
                    Error(
                        "ScoreJSONReader: a \"%\" direction cannot carry \"%\". Only "
                        "tempoRamp uses endpoints. Got %"
                        .format(entry["type"], key, entry)
                    ).throw
                }
            };
            ^this
        ...
        ["edge", "id"].do { |key|
            if (entry[key].isNil) {
                Error(
                    "ScoreJSONReader: a tempoRamp direction needs \"%\". Got %"
                    .format(key, entry)
                ).throw
            }
        };
        if (["start", "stop"].any { |each| each == entry["edge"] }.not) {
            Error(
                "ScoreJSONReader: \"%\" is not a tempo ramp edge. Use start or stop."
                .format(entry["edge"])
            ).throw
        };
        ^this
    ...
    *prIsRampStop { |entry|
    ...
    // Answers one Tuplet with authored bracket counts if present.
    // Both counts or neither. They must match the multiplier.
    *tuplet { |dict, elements|
        ...
        if (actual.isNil or: { normal.isNil }) {
            Error(
                "ScoreJSONReader: a tuplet needs both \"actualNotes\" and "
                "\"normalNotes\", or neither."
            ).throw
        };
        ...
        if (Duration(normal, actual) != multiplier) {
            Error(
                "ScoreJSONReader: tuplet counts %:% do not match multiplier %."
                .format(actual, normal, multiplier)
            ).throw
        };
    ...
    // constructor call.
    //
    // >>> ScoreJSONReader.marking(Dictionary["type" -> "text", "value" -> "solo", "placement" -> "below"]).placement
    // below
    *marking { |entry|
        ...
        if (kind != \text) {
            if (placement.notNil) {
                Error(
                    "ScoreJSONReader: a % marking cannot carry \"placement\"."
                    .format(kind)
                ).throw
            };
        ...
        if (entry["value"].isKindOf(String).not) {
            Error(
                "ScoreJSONReader: text must be a string, got %"
                .format(entry["value"])
            ).throw
        };
        if (placement.isNil) {
            Error(
                "ScoreJSONReader: a text marking needs \"placement\" above or below."
            ).throw
        };
    ...
    // way. It has to: parseJSON maps both an absent key and an
    ...
    *boolean { |value, what|
        ...
        Error(
            "ScoreJSONReader: % must be true or false, got %"
            .format(what, value)
        ).throw
    }
    ...
    // the wire. An interchange contract that admits two spellings
    ...
    *tieMask { |value|
        ...
        Error(
            "ScoreJSONReader: a chord's tiesToNext must be one flag per pitch, got %."
            .format(value)
        ).throw
    }
