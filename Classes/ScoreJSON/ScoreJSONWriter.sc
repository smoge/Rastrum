...
//
// The envelope and schema constants live in ScoreJSONContract.
// The class methods here stay as the public writer-facing API.
ScoreJSONWriter : ScoreWriter {
    ...
    *schemaVersion { ^ScoreJSONContract.schemaVersion }
    ...
    write { |element|
        if (element.isKindOf(MusicScore).not) {
            Error(
                "ScoreJSONWriter: expected a MusicScore document, got a %. Wrap "
                "fragments in MusicScore before writing.".format(element.class)
            ).throw
        };
    ...
    *escape { |string|
    ...
    // Written only when there are any and in the order the leaf
    // holds them. A dynamic before an articulation isn't the same
    // instruction as the reverse, so the wire keeps that order. Two
    // articulations are one fact and arrive already sorted.
    ...
    writeGraces { |leaf|
        ...
        leaf.graces.do { |grace|
            ...
            if (grace.isKindOf(MusicNote).not and: { grace.isKindOf(Chord).not }) {
                Error(
                    "ScoreJSONWriter: a grace group may hold only notes and chords, got a %.".format(grace.class)
                ).throw
            };
            ...
            if (tied or: { grace.hasMarkings } or: { grace.hasSpanners }
                or: { grace.hasGraces }) {
                Error(
                    "ScoreJSONWriter: a grace leaf cannot carry ties, markings, spanners or nested graces."
                ).throw
            };
            // Grace durations must already be single note heads.
            if (grace.dur.isNotatable.not) {
                Error(
                    "ScoreJSONWriter: grace duration % must be writable as one note head.".format(grace.dur)
                ).throw
            };
        };
