...
// Rewriting is textual, so Strings, symbols, character literals and comments
// are copied through. A block is `[tag|` with no space and a closing `|]`. The
// sclang array `[a | b]` is not one.

// RastrumQuasiquoter: tagged score-notation blocks in source text.
//
...
RastrumQuasiquoter {
    ...
    *translate { |source|
        ...
        if (source.isKindOf(String).not) {
            Error(
                "RastrumQuasiquoter.translate: source must be a String, got % (%)."
                .format(source, source.class)
            ).throw
        };
    ...
    *prOpensATag { |source|
    ...
    // The end of quoted/comment text starting at `index`, or nil. Blocks open
    // only outside quoted text and comments.
    ...
    *prBodyEnd { |source, start, tag, where|
        ...
        while { i < size } {
            ...
            { char == quote } {
                ...
                if (closed.isNil) {
                    Error(
                        "RastrumQuasiquoter: [%| ... |] % leaves a quotation open. "
                        "Close it or remove it.".format(tag, where)
                    ).throw
                };
                i = closed;
            }
        ...
        Error(
            "RastrumQuasiquoter: [%| ... |] % never closes. A block ends with |].".format(tag, where)
        ).throw
    }
    ...
    *prPadding { |consumed|
    ...
    // Block errors are refused here and get a line/column. Notation errors
    // come from the parser the expansion calls, with the same generated code
    // as the source-level call.
    *prExpand { |tag, body, where|
        ...
        Error(
            "RastrumQuasiquoter: % is in tags and has no expansion."
            .format(tag.asCompileString)
        ).throw
    }
    ...
    *prMeasure { |text, where|
        ...
        if (split.isNil) {
            Error(
                "RastrumQuasiquoter: [measure| ... |] % states no meter. Use [measure| 4/4 c4 d4 e2 |].".format(where)
            ).throw
        };
        if (split[0].isEmpty or: { split[1].isEmpty }) {
            Error(
                "RastrumQuasiquoter: [measure| ... |] % has no %. Use [measure| 4/4 c4 d4 e2 |]."
                .format(
                    where,
                    if (split[0].isEmpty) { "meter" } { "bar" }
                )
            ).throw
        };
        if (split[2] ? false) {
            ^"Measure.notation(%)".format(this.prQuoted(text))
        };
    ...
    *start { |force = false|
        ...
        if (current.notNil and: { force.not }) {
            Error(
                "Rastrum: the interpreter already has a preprocessor. Stop it first, or call Rastrum.startQuasiquoter(true) to replace it."
            ).throw
        };
        saved = current;
    ...
    *stop {
        if (this.active.not) {
            if (installed.notNil) {
                warn("Rastrum: the quasiquoter is no longer the installed preprocessor, so something else replaced it. Restoring ould take that away. Nothing changed.")
            };
            saved = nil;
            installed = nil;
            ^this
