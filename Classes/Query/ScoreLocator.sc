// ScoreLocator: time queries over score rows
//
// `ScoreEventStream` and `ScoreDirectionStream` flatten the score. This asks
// one moment or window of those rows.
//
...
// Two windowing rules:
//
//     a marking, a spanner endpoint, a direction   offset in [start, stop)
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
// Every path this class answers addresses `source`. By default `source` is a
// prepared copy, and preparation may change leaf paths. A prepared path can
// name another leaf in the original tree, or none at all.
//
...
// carries source. Path methods can't, because a path doesn't.
ScoreLocator {
    ...
    *new { |element, prepare = true|
    ...
    // Measure directions written exactly here.
    //
    // ^ [IdentityDictionary]
    directionsAt { |offset|
        var here = Duration.asDuration(offset);
    ...
    // sounds.
    //
    // See Note [A prepared address is not an authored one].
    //
    ...
    // Every selected leaf overlapping the window, using `ScoreSelection`'s rule.
    //
    // ^ ScoreSelection
    ...
    prCheckWindow { |start, stop|
        ...
        if (to <= from) {
            Error(
                "ScoreLocator: window % to % has no length. A window is half open, so it needs a stop after its start."
                .format(from, to)
            ).throw
        };
        ^[from, to]
