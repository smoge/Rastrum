...
// The façade.
//
// Most methods are one line over the model, writers or playback
// layers.
//
// Only `render` runs external binaries.
Rastrum {
    ...
    *version { ^"0.1.0" }
    ...
    // Set by hand when discovery missed or found the wrong binary.
    // `nil` means no LilyPond.
    *lilypondPath_ { |path|
        if (path.notNil and: { File.exists(path).not }) {
            Error(
                "Rastrum: lilypondPath must be a file path or nil, got %."
                .format(path.asCompileString)
            ).throw
        };
        lilypondPath = path;
        ^this
    ...
    // Use a login shell on Unix so Homebrew paths are visible.
    // Windows uses `where`.
    *findLilypond {
    ...
    *writeMusicXML { |element, name, prepare = true, beam = true|
        if (element.isKindOf(MusicScore).not) {
            Error(
                "Rastrum: writeMusicXML needs a MusicScore document, got %."
    ...
    // No `beam`.
    // See Note [A derived beam is not a score fact]. An
    ...
    *readJSON { |path|
    ...
    //
    // `profile` is optional and is where interpretation enters.
    // See Note [One public path, one stated order].
    ...
    *prCheckedProfile { |profile|
        ...
        if (profile.isKindOf(PlaybackProfile).not) {
            Error(
                "Rastrum: profile takes a PlaybackProfile or nil, got %. Build one with "
                "PlaybackProfile.new and its map slots."
                .format(profile.class.name)
            ).throw
        };
        ^profile
    }
    ...
    *render { |element, name, compile = true, open = true, prepare = true,
        ...
        if (lilypondPath.isNil) {
            Error(
                "Rastrum: LilyPond was not found on PATH. Install it or set:\n    "
                "Rastrum.lilypondPath = \"/path/to/lilypond\"\nThe .ly file was still written at %."
                .format(lyPath)
            ).throw
        };
    ...
    *openPDF { |path|
        if (path.isNil or: { File.exists(path).not }) {
            Error(
                "Rastrum: PDF not found at %. Render first or pass an existing PDF "
                "path."
                .format(path.asCompileString)
            ).throw
        };
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // `LilyWriter` follows `Rastrum.lilypondVersion`. It doesn't
    ...
    *checkLilypondSpeaksTheFile { |element, installed|
        ...
        Error(
            "Rastrum: grouped-meter spelling mismatch. Declared LilyPond % uses %, "
            "installed % uses %. Set Rastrum.lilypondVersion to \"%\" or use a matching LilyPond binary."
            .format(
                lilypondVersion,
    ...
    *prQuotedFor { |path, windows|
    ...
    // LilyPond writes to stderr on success and failure, so status is the
    // verdict. `systemCmd` answers a wait status, not an exit code: exit 1
    // arrives as 256. Report raw status so shell failures stay searchable.
    *prCheckedRun { |status, lyPath|
        if (status != 0) {
            Error(
                "Rastrum: LilyPond failed with status % on %. See the post window. No PDF was opened."
                .format(status, lyPath)
            ).throw
        };
        ^status
    }
