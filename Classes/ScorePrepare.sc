...
//
// `run` returns a fresh tree with writer-facing notation facts made
// explicit. Measuring helpers expose bar-local leaf offsets and
// written-time scale.
//
...
ScorePrepare {
    ...
    // Sounding time applies every enclosing multiplier. Metric
    // boundaries use sounding time. Note heads and bracket cuts use
    // written time. The walk stores both so nested splits don't
    // rederive the tree.

    // Exact sounding offsets from the bar's metric origin. Pickups
    // start late in their notional bar.
    //
    // >>> { |m| m.leaves.collect { |x| ScorePrepare.leafOffsetsIn(m)[x] } }.value(RhythmTree.measure(Meter(4, 4), [1, 1, 1, 1]))
    ...
    *leafMultipliersIn { |measure|
    ...
    // A fresh notation-prepared tree: one-notehead leaves, split
    // fragments tied onward. Explicit and facade-assisted: the façade
    // calls this before writing and no writer calls it.
    // Syntax-specific ceilings stay in writers.
    //
    ...
    *run { |element|
        ...
        Error(
            "ScorePrepare.run: expected a MusicScore, Staff or Measure, got a %."
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // Staff preparation walks each timeline across authored bar
    // spans. Overflow moves forward. Underfull bars are refused
    // unless previous overflow fills them. Voices match by position.
    // Optional names must agree.
    //
    // Containers crossing a barline are split and rewrapped in each
    // bar.
    ...
    *prPrepareStaff { |staff|
        ...
        measures.do { |bar|
            if (bar.isKindOf(Measure).not) {
                Error(
                    "ScorePrepare: Staff may hold only measures, got a %."
                    .format(bar.class)
                ).throw
            };
    ...
    *prVoiceCount { |measures, flat|
        ...
        measures.do { |bar|
            if (bar.hasVoices.not) {
                Error(
                    "ScorePrepare: a % bar has no voices, but other bars in this staff do."
                    .format(bar.meter)
                ).throw
            }
        };
        ...
        if (counts.size != 1) {
            Error(
                "ScorePrepare: this staff has % different voice counts."
                .format(counts.size)
            ).throw
        };
                ...
                if (voice.name.notNil) {
                    if (names[j].isNil) {
                        ...
                        if (names[j] != voice.name) {
                            Error(
                                "ScorePrepare: voice % is named \"%\" in one bar and \"%\" in another."
                                .format(j + 1, names[j], voice.name)
                            ).throw
                        }
                    }
                }
    ...
    *prLayOut { |measures, count, names, flat|
        ...
        count.do { |j|
            var barIndex = 0, position = Duration(0, 1);
                // Overflow moves forward. Short bars cannot borrow ahead.
            ...
            measures.do { |authored, i|
                ...
                if (barIndex == i) {
                    Error(
                        "ScorePrepare: bar % declares % but is short by %%."
                        .format(
                            i + 1,
                            spans[i],
                            spans[i] - position,
                            if (count > 1) { " in voice " ++ (j + 1) } { "" }
                        )
                    ).throw
                }
            }
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // Preparation never mutates the source tree. `run` returns fresh
    // elements. Reusing children would repoint parents.
    //
    ...
    *rebuilt { |element, children|
        ...
        Error(
            "ScorePrepare.rebuilt: expected a container, got a %."
            .format(element.class)
        ).throw
    }
    ...
    // time.
    //
    // See Note [Written time and sounding time].
    ...
    *prPrepareMeasure { |measure|
        ...
        if (measure.isFull.not) {
            Error(
                "ScorePrepare: a % bar declares % but holds %. A lone measure must already fit its own span."
                .format(measure.meter, measure.barDuration, measure.duration)
            ).throw
        };
    ...
    *prRebuildAll { |children, measure, offsets, multipliers|
    ...
    // Adjacent plain rests in one timeline are one silence. Rewrite
    // them as the fewest readable rests at that offset.
    //
    // Rests never tie. Attached rests aren't plain, so their position
    // is preserved.
    ...
    *prIsPlainRest { |element|
    ...
    // The fewest rests that spell this run's span where the run begins.
    *prRestRun { |run, measure, offsets, multipliers|
        ...
        pieces = if (multipliers[first] == Duration(1, 1)) {
            ...
            if (written.isNotatable) { [written] } {
                written.tieRuns ?? {
                    Error(
                        "ScorePrepare: rest % inside a tuplet cannot be split into "
                        "notatable pieces."
                        .format(written)
                    ).throw
                }
    ...
    *prSplitLeaf { |leaf, meter, offset, multiplier|
        ...
        pieces = if (multiplier == Duration(1, 1)) {
            ...
            leaf.dur.tieRuns ?? {
                Error(
                    "ScorePrepare: % inside a tuplet cannot be split into tied "
                    "notatable pieces."
                    .format(leaf.dur)
                ).throw
            }
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // Markings and grace groups belong to the attack, so only the
    // first fragment keeps them. Spanner starts stay on the first
    // fragment. Ordinary stops stay on the last. Stops that name the
    // attack stay on the first: glissando stops and stops on a leaf
    // that also starts the same kind.
    //
    // The last fragment inherits the original forward tie. Earlier
    // fragments tie onward.

    // Copy one split fragment.
    *prCopyLeaf { |leaf, dur, tieSource, isFirst = true|
        var mask, copy;
        copy = case
        ...
        // See Note [Split leaf adornments].
        copy.spanners_(leaf.spanners.select { |endpoint|
            if (this.prNamesTheAttack(leaf, endpoint)) {
                isFirst
            } {
                tieSource.notNil
            }
    ...
    // Endpoints copied to the attacking fragment.
    *prNamesTheAttack { |leaf, endpoint|
        if (endpoint.isStart or: { endpoint.isGlissando }) { ^true };
        ^leaf.spanners.any { |each|
            each.isStart and: { each.kind == endpoint.kind } }
    }

    // The meter decides the cut lines.
    //
    // Split at the strongest crossed metric line, then recurse. Notes
    // settle when notatable. A rest is placed by the meter.
    //
    // See Note [A rest carries the beat, a note carries the line] in Meter.sc.
    //
    // Meter owns the hierarchy.
    //
    // See Note [The metric hierarchy belongs to Meter] in Meter.sc.
    *prMetricPieces { |offset, dur, meter, depth, forRest = false|
        ...
        if (depth > 16) {
            Error(
                "ScorePrepare: gave up splitting % at offset % in %."
                .format(dur, offset, meter)
            ).throw
        };
        point = meter.strongestInteriorPoint(offset, dur);
        if (point.isNil) {
            Error(
                "ScorePrepare: % at offset % in % crosses no writable metric line."
                .format(dur, offset, meter)
            ).throw
        };
        acc = List.new;
    ...
    *prRequireMeasure { |element, method|
        if (element.isKindOf(Measure).not) {
            Error(
                "ScorePrepare.%: expected a Measure, got a %."
                .format(method.name, element.class)
            ).throw
        };
        if (element.mixesVoicesWithElements) {
            Error(
                "ScorePrepare.%: a % bar mixes voices with loose elements."
                .format(method.name, element.meter)
            ).throw
        };
        ^this
    }
