...
//
// Playback side-effect boundary.
//
// `playable` answers the pattern to play. `play` starts it.
PatternPlayback {

    // Overlay only the explicit overrides. `nil` for both answers `pattern`.
    //
    // `Pbindf` overrides. There is no instrument or amp default here.
    // SC default event already has them. An amp override flattens
    // every loudness key.
    //
    // >>> { |p| PatternPlayback.playable(p, nil, nil) === p }.value(Pbind(\dur, 1))   -> true
    ...
    *playable { |pattern, instrument, amp|
    ...
    // Width already on the event; flat default otherwise.
