// PlaybackEnvelopeMap: shaped control values over events.
//
// One control moving over the length of one event.
// This is the layer for what a scalar can't say: pan across a held note,
// cutoff through a tied chord or any control whose value is a shape.
//
// Playback-only.
//
// `PlaybackProfile` composes it with the other layers.
PlaybackEnvelopeMap {
    ...
    *keysFor { |control|
    ...
    // Note [Playback envelopes are fixed-width control values]
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // A bare Array in an event key is a multichannel request.
    //
    // This map accepts plain arrays and `Ref`-wraps them at output.
    //
    // The SynthDef must declare `<control>Levels` with `segments + 1` channels and
    // `<control>Times` with `segments`.
    //
    // A timeline with an envelope carries the keys on every event. Uncovered
    // events hold the value in force, padded to the same SynthDef width, so
    // `PatternWriter` sees no nil that would stop the stream.

    // Every event of one timeline takes this envelope.
    //
    // `levels` and `times` are arrays and are Ref-wrapped on output.
    //
    // `times` are beats.
    //
    ...
    // Note [Event-local envelopes target sounding written events]
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // A partly tied chord may answer several events at one written
    // offset. An offset envelope reaches all of them and must fit
    // each one.
    //
    // `envelopeAtOffset` targets the written note. Inserted graces at
    // that offset are stepped over. Timeline-wide envelopes still
    // reach ornaments.
    //
    // A rest event can carry envelope keys, but it makes no synth. It
    // can't be an event-local target.
    //
    // An event-local envelope must fit before the synth frees.
    // Shorter envelopes finish and hold their last level.
    // Timeline-wide envelopes aren't measured against each event.
    ...
    // Segment width for this control; nil when this map does not write it.
    //
    ...
    //
    // >>> PlaybackEnvelopeMap.new.envelopeAt(0, 0, \pan, [-1, 1], [1]).events(Measure("1/4", "c4")).first.includesKey(\panLevels)
    // true
    ...
    // caller's events alone. What `PlaybackProfile` reaches for.
    eventsFrom { |events|
    ...
    prBaselineOf { |event, control, here|
        ...
        Error(
            "PlaybackEnvelopeMap: staff % timeline % has an envelope for % on part of "
            "it and no value for the rest. Add baseline(%, value) or a constant "
            "PlaybackControlMap value."
            .format(
                here[0],
                here[1],
                control,
                control.asCompileString
            )
        ).throw
    }

    // Check targets and event-local fit before writing any keys.
    // See Note [Event-local envelopes target sounding written events].
    ...
    prCheckEntry { |entry, found|
        ...
        if (inTimeline.isEmpty) {
            Error(
                "PlaybackEnvelopeMap: this score has no staff % timeline %, so the % "
                "envelope reaches nothing."
                .format(here[0], here[1], entry[\control])
            ).throw
        };
        ...
        if (atMoment.isEmpty) {
            Error(
                "PlaybackEnvelopeMap: no event of staff % timeline % begins at %, so "
                "the % envelope reaches nothing."
                .format(here[0], here[1], at, entry[\control])
            ).throw
        };
        if (sounding.isEmpty) {
            ...
            if (atMoment.any { |each| each[\rest] }) {
                Error(
                    "PlaybackEnvelopeMap: staff % timeline % rests at %, and the % "
                    "envelope has no synth to move."
                    .format(here[0], here[1], at, entry[\control])
                ).throw
            };
            Error(
                "PlaybackEnvelopeMap: the only thing staff % timeline % has at % is an "
                "inserted ornament. Target a written note."
                .format(here[0], here[1], at)
            ).throw
        };
        sounding.do { |target| this.prCheckFits(entry, target) };
    }

    // Both sides are beats. Slack is only for floating-point sums.
    // See Note [Event-local envelopes target sounding written events].
    prCheckFits { |entry, target|
        ...
        if (span > (target[\dur] + 1e-9)) {
            Error(
                "PlaybackEnvelopeMap: the % envelope at % lasts % beats and the event "
                "lasts %. Shorten it or target a longer event."
                .format(entry[\control], target[\offset], span, target[\dur])
            ).throw
        }
    }
    ...
    prPayloadOf { |event, index|
        ...
        if (payload.isNil) {
            Error(
                "PlaybackEnvelopeMap: event % has no \\rastrum payload. Use EventWriter."
                .format(index)
            ).throw
        };
        [\staffIndex, \timelineIndex, \offset].do { |key|
            if (payload[key].isNil) {
                Error(
                    "PlaybackEnvelopeMap: event % payload has no %. Use EventWriter."
                    .format(index, key)
                ).throw
            }
        };
    ...
    prCheckedTarget { |staffIndex, timelineIndex|
        [staffIndex, timelineIndex].do { |value, position|
            if (value.isKindOf(Integer).not or: { value < 0 }) {
                Error(
                    "PlaybackEnvelopeMap: % index must be a non-negative Integer, got %."
                    .format(
    ...
    // A control this map may move.
    //
    // Structural keys, payload names, `\amp`, `\tempo` and derived keys are
    // refused. See Note [Loudness as breakpoints] in PlaybackMapLoudness.sc.
    // Loudness envelopes belong to `PlaybackMap`.
    prCheckedControl { |control|
        ...
        if (reserved.includes(name)) {
            Error(
                "PlaybackEnvelopeMap: % is reserved and cannot be enveloped."
                .format(name)
            ).throw
        };
        PlaybackEnvelopeMap.keysFor(name).do { |key|
            if (reserved.includes(key)) {
                Error(
                    "PlaybackEnvelopeMap: an envelope for % would write %, which is reserved."
                    .format(name, key)
                ).throw
            }
        };
    ...
    prCheckedLevels { |levels, times, control|
        ...
        if (list.size < 2) {
            Error(
                "PlaybackEnvelopeMap: an envelope for % needs at least two levels."
                .format(control)
            ).throw
        };
        if (list.size != wanted) {
            Error(
                "PlaybackEnvelopeMap: an envelope for % needs levels = times + 1, got % and %."
                .format(
                    control,
                    list.size,
                    wanted - 1
    ...
    prCheckedTimes { |times, control|
        ...
        list.do { |each|
            if (each < 0) {
                Error(
                    "PlaybackEnvelopeMap: a segment of an envelope for % lasts % beats. "
                    "Times must be zero or more."
                    .format(control, each)
                ).throw
            }
        };
    ...
    prCheckedNumbers { |values, what, control|
        ...
        if (values.isKindOf(Ref)) {
            Error(
                "PlaybackEnvelopeMap: the % of an envelope for % are written as a plain array, not a Ref."
                .format(what, control)
            ).throw
        };
        // Ordered. A String is sequenceable without being a numeric list.
        // An envelope is a shape, so a Set or Dictionary would use
        // whichever traversal order it happens to hold.
        if (values.isSequenceableCollection.not
            or: { values.isKindOf(String) }) {

            Error(
                "PlaybackEnvelopeMap: the % of an envelope for % must be an ordered "
                "Array of numbers, got %."
                .format(what, control, values.asCompileString)
            ).throw
        };
        list = values.asArray;
        list.do { |each|
            if (each.isNumber.not) {
                Error(
                    "PlaybackEnvelopeMap: % in % for % is not a number."
                    .format(each.asCompileString, what, control)
                ).throw
            }
        };
        ^list
    ...
    prCheckedNumber { |value, what|
        if (value.isNumber.not) {
            Error(
                "PlaybackEnvelopeMap: % is %, not a number."
