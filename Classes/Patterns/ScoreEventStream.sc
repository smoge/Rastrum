...
ScoreEventStream {
    ...
    *initClass {
    ...
    // A guarded copy, matching `PlaybackMap`'s tables.
    //
    ...
    *records { |element, prepare = true|
    ...
    // The same rows over events somebody already has.
    *recordsFromEvents { |events|
    ...
    //
    // >>> ScoreEventStream.between(ScoreEventStream.records(Measure("2/4", "c4 d4")), 0, "4").size
    // 1
    *between { |records, start, stop|
        var from = Duration.asDuration(start), to = Duration.asDuration(stop);
        if (to <= from) {
            Error(
                "ScoreEventStream.between: window % to % has no length."
                .format(from, to)
            ).throw
        };
    ...
    //
    // >>> ScoreEventStream.inTimeline(ScoreEventStream.records(Measure("2/4", "c4 d4")), 0, 0).size
    // 2
    *inTimeline { |records, staffIndex, timelineIndex|
    ...
    // >>> ScoreEventStream.ofKind(ScoreEventStream.records(Measure("2/4", "c4 d4")), \event).size
    // 2
    ...
    *prAttachmentRow { |payload, kind, key, record|
    ...
    // Location fields shared by every row kind.
    *prWhere { |payload|
    ...
    *prPayloadOf { |event, index|
        ...
        if (payload.isNil) {
            Error(
                "ScoreEventStream: event % has no \\rastrum payload. Use EventWriter."
                .format(index)
            ).throw
        };
        requiredKeys.do { |key|
            if (payload[key].isNil) {
                Error(
                    "ScoreEventStream: event % payload has no %. Use EventWriter."
                    .format(index, key)
                ).throw
            }
        };
