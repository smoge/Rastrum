...
// System text isn't guessed. Nothing is per voice. A direction
// attaches to a bar.


// ScoreDirectionStream: a score's measure directions as flat rows,
// for reading rather than playing.
//
...
ScoreDirectionStream {
    ...
    *initClass { scoreWideKinds = [\tempo, \rehearsalMark, \tempoRamp] }
    ...
    // A guarded copy, matching `ScoreEventStream.requiredKeys`.
    //
    ...
    *scoreWideKinds { ^scoreWideKinds.copy }
    ...
    //
    // >>> ScoreDirectionStream.records(Measure("2/4", "c4 d4").rehearsalMark("A")).size
    // 1
    *records { |element, prepare = true|
    ...
    //
    // >>> ScoreDirectionStream.between(ScoreDirectionStream.records(Measure("2/4", "c4 d4").text("solo", "4")), 0, "2").size
    // 1
    *between { |records, start, stop|
        var from = Duration.asDuration(start), to = Duration.asDuration(stop);
        if (to <= from) {
            Error(
                "ScoreDirectionStream.between: window % to % has no length."
                .format(from, to)
            ).throw
        };
    ...
    *ofDirectionKind { |records, kind|
    ...
    //
    // Refuse missing staff indexes when row metadata has a count.
    //
    ...
    *prCheckStaffIndex { |count, staffIndex|
        if (staffIndex.isKindOf(Integer).not
            ...
            Error(
                "ScoreDirectionStream.inStaff: this score has % staff/staves, so staff % does not exist."
                .format(count, staffIndex.asCompileString)
            ).throw
        }
    }
    ...
    *prRefuseConflict { |entry|
        Error(
            "ScoreDirectionStream: two score-wide directions at %, % and %. A score-wide moment needs one reading."
