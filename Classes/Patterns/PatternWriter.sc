...
PatternWriter {
    ...
    *initClass {
    ...
    // `\legato`.
    //
    // Argument-local, not mutable class state.
    //
    ...
    *prRastrumOf { |event|
        ...
        if (payload.isNil) {
            Error(
                "PatternWriter: event has no \\rastrum payload. Use EventWriter or "
                "provide complete Rastrum metadata."
            ).throw
        };
        [\staffIndex, \timelineIndex].do { |key|
            if (payload[key].isNil) {
                Error(
                    "PatternWriter: event \\rastrum payload has no %. Use EventWriter "
                    "or provide complete timeline metadata."
                    .format(key)
                ).throw
            }
        };
    ...
    *prPbindFor { |events, carried|
        ...
        (carried ? keys).do { |key|
            ...
            case
                ...
                { true } {
                    Error(
                        "PatternWriter: % is present on some events of a timeline and "
                        "absent on others. A carried Pbind key must be all present or "
                        "all absent."
                        .format(key)
                    ).throw
                };
    ...
    // A rest has no pitch. Nil would end the stream. Use SC's `Rest`.
