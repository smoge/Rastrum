...
// one. `EventWriter` stays structural. `PlaybackProfile` composes
// this layer.
//
// A grace leaf's written duration is display value only. This layer
// chooses the playback length. A plain grace borrows from the
// previous event and sounds before the beat. An acciaccatura borrows
// from its host and sounds on the beat. Written offsets do not move;
// only top-level `dur` changes.
PlaybackGraceMap {
    var length;
    ...
    *new { ^super.new.init }
    ...
    length_ { |value|
        ...
        if (checked <= Duration(0, 1)) {
            Error(
                "PlaybackGraceMap: grace length must be positive, got %."
                .format(checked)
            ).throw
        };
        length = checked;
    ...
    //
    // >>> PlaybackGraceMap.new.events(Measure("2/4", [MN("c4").acciaccatura(MN("b8")), MN("d4")])).size
    // 3
    ...
    // One Ppar over the spliced timelines. This layer adds no keys of
    // its own.
    ...
    prRefusePeer { |map|
        Error(
            "PlaybackGraceMap: compose playback layers with PlaybackProfile, not this method. Got %."
    ...
    //
    // Events are copied first. `remaining` tracks what each lender
    // still has.
    //
    ...
    // One group. Refuses no previous event, not enough duration or a
    // graced continuation.
    ...
    prSpliceBefore { |events, record, remaining, at, borrowed|
        ...
        lender[\dur] = EventWriter.prBeats(left - borrowed);
    ...
    prSpliceOnBeat { |events, record, remaining, at, borrowed|
        ...
        host[\dur] = EventWriter.prBeats(left - borrowed);
    ...
    // Where the host sounds. Use the first non-grace event at that
    // place.
    ...
    prGraceEvents { |leaf, host, shift|
        ...
        ^leaf.graces.collect { |grace, index|
            ...
            event[\dur] = EventWriter.prBeats(length);
    ...
    prRefuseStyle { |style|
        Error(
            "PlaybackGraceMap: unsupported grace style %."
            .format(style.asCompileString)
        ).throw
    }

    prRefuseFirst { |record|
        Error(
            "PlaybackGraceMap: the grace at % begins staff % timeline %, so there is no "
            "previous event to borrow from. Use acciaccatura for on-beat borrowing."
            .format(
                record[\offset],
                record[\staffIndex],
                record[\voiceIndex]
    ...
    prRefuseBorrow { |record, borrowed, remaining|
        Error(
            "PlaybackGraceMap: the grace group at % wants % and the note it borrows "
            "from has % left. Shorten the group or grace length."
            .format(record[\offset], borrowed, remaining)
        ).throw
    }

    prRefuseUnmatched { |record|
        Error(
            "PlaybackGraceMap: the grace at % is not on an attack. It is on a tied continuation."
