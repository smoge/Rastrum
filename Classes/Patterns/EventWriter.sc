...
// read it. Playback targets are `[staffIndex, timelineIndex]`.
// Staff and voice names are labels only. `dur` is a Float in SC beats.
...
EventWriter {
    ...
    *events { |element|
    ...
    // Exact whole-note time as SuperCollider beats.
    //
    // Any exact span, not only a note's length: event durations,
    // payload offsets, ramp spans, grace lengths and tempo-lane
    // silences all cross here. This is the unit boundary the playback
    // layers cross, so they ask rather than each spelling the same
    // multiplier. See Note [The event payload is structural].
    //
    // >>> EventWriter.prBeats(Duration(1, 4))   -> 1.0
    // >>> EventWriter.prBeats(Duration(1, 1))   -> 4.0
    ...
    *prCollect { |element, offset, multiplier, measureIndex, voiceName, found|
    ...
    // pitch streams first. Runs with the same offset and duration
    // join back into one event. A tied continuation doesn't attack,
    // but can carry markings or spanner endpoints. Each run keeps the
    // leaves it crossed.
    ...
    *prSoundingRuns { |entries|
        ...
        if (pending.notEmpty) {
            Error(
                "EventWriter: % ties onward with no following note. Validate the score before writing events."
                .format(pending.keys.asArray)
            ).throw
        };
        ^runs.asArray
    }
    ...
    *prPitchesOf { |leaf|
    ...
    // One flag per pitch, in pitch order. Notes read as one-pitch
    // chords.
    *prTiesOf { |leaf|
    ...
    *prRequireContinuations { |pending, pitches, leaf|
        pending.keysDo { |pitch|
            if (pitches.any { |arriving| arriving == pitch }.not) {
                Error(
                    "EventWriter: % ties onward into %, which does not contain that pitch."
                    .format(pitch, leaf)
                ).throw
            }
        };
    ...
    *prGroupRuns { |runs, staffName, staffIndex, timelineIndex|
    ...
    // Ordered by onset, then low-to-high for simultaneous attacks.
    *prOrdered { |events|
    ...
    *prLowestOf { |event|
    ...
    // offset.
    //
    // See Note [The event payload is structural].
    //
    ...
    *prEventFor { |group, staffName, staffIndex, timelineIndex|
        ...
        // The one lossy value. The exact Duration travels in the
        // payload.
        event[\dur] = this.prBeats(first[\duration]);
        payload[\duration] = first[\duration];
        payload[\offset] = first[\offset];
        // Names are labels. Indexes are playback targets.
