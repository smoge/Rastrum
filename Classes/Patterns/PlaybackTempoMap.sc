...
// their own list.
// See Note [Playback interpretation is explicit] in Patterns/PlaybackMap.sc.
// Prose is notation. A number is an interpretation.
// `Direction.tempo("Allegro")` stays prose. Named speeds live in this table.
// Unmapped prose is refused, not guessed.
PlaybackTempoMap {
    ...
    *new { ^super.new.init }
    ...
    // >>> PlaybackTempoMap.new.tempo("Andante", beat: "8", bpm: 144).tempos["Andante"]
    // 72.0
    ...
    //
    // >>> PlaybackTempoMap.new.records(Measure("1/4", "c4").metronome("4", 120)).first[\bpm]
    // 120.0
    ...
    // Tempo changes as silent SC Events. `\tempo` is beats per second. Score
    // tempi are beats per minute.
    //
    // A lead-in carries duration before the first written change. Without it, a
    // tempo over bar five would start at bar one. A score with no tempo
    // directions gets one inert, zero-length rest so the pattern is schedulable.
    //
    // Ramp steps use the same list. The tree is prepared once so point records
    // and ramp endpoints share offsets.
    ...
    *eventsFrom { |records|
        ...
        if (records.first[\offset] > Duration(0, 1)) {
            out.add(this.prSilence(EventWriter.prBeats(records.first[\offset])))
        };
        records.do { |record, index|
            var next = records[index + 1];
            var beats = next !? {
                EventWriter.prBeats(next[\offset] - record[\offset])
            } ? 0;
    ...
    *withScoreTempo { |pattern, tree|
    ...
    //
    // >>> PlaybackTempoMap.scoreLaneRecords(Measure("1/4", "c4").metronome("4", 120)).first[\bpm]
    // 120.0
    *scoreLaneRecords { |tree|
    ...
    // Tempo events as one pattern, to sit beside the music.
    //
    ...
    *prStepsOf { |ramp|
        ...
        var beats = EventWriter.prBeats(span);
    ...
    *prMatched { |start, stop, source|
    ...
    // speed that ramp starts from. A ramp opening where another
    ...
    *prRefuseNoTarget { |record|
        Error(
            "PlaybackTempoMap: the tempo ramp % at % says how it changes but not what "
            "it changes to. Give the ramp start a target bpm or omit it from playback."
            .format(this.prRampName(record), record[\offset])
        ).throw
    }

    *prRefuseNoSource { |record|
        Error(
            "PlaybackTempoMap: the tempo ramp % at % has no speed to start from. Add a "
            "prior metronome mark or map the preceding tempo text."
            .format(this.prRampName(record), record[\offset])
        ).throw
    }
    ...
    *prRefuseBackwards { |start, stop|
        Error(
            "PlaybackTempoMap: the tempo ramp % starts at % and stops at %, so it spans "
            "no time."
            .format(this.prRampName(start), start[\offset], stop[\offset])
        ).throw
    }

    *prRefuseOverlap { |record|
        Error(
            "PlaybackTempoMap: a tempo ramp is still open when % starts at %. "
            "Overlapping tempo ramps are not supported."
            .format(this.prRampName(record), record[\offset])
        ).throw
    }
    ...
    *prRefuseUnmatched { |record|
        Error(
            "PlaybackTempoMap: a tempo ramp stop with id % at % closes nothing."
            .format(record[\id], record[\offset])
        ).throw
    }

    *prRefuseDangling { |record|
        Error(
            "PlaybackTempoMap: the tempo ramp % at % is never closed, so there is no "
            "target moment."
            .format(this.prRampName(record), record[\offset])
        ).throw
    }
    ...
    *directionsIn { |element, prepare = true|
    ...
    // The direction walk, over tempo-ramp endpoints.
    ...
    *prRefuseConflict { |entry|
        Error(
            "PlaybackTempoMap: two tempo directions at %, % and %. One moment cannot "
            "have two tempo readings."
    ...
    prRefuseUnmapped { |record|
        Error(
            "PlaybackTempoMap: this score is marked \"%\" at % and this map has no "
            "speed for it. Known text tempi: %. Add tempo(\"%\", bpm), or write a "
            "metronome mark."
            .format(
                record[\text],
                record[\offset],
    ...
    prOneName { |kept, alias, keptName, aliasName|
        if (kept.notNil and: { alias.notNil }) {
            Error(
                "PlaybackTempoMap: use % or %, not both."
                .format(keptName, aliasName)
            ).throw
        };
        ^kept ?? { alias }
    }
    ...
    prCheckedBpm { |bpm, text|
        if (bpm.isNumber.not or: { bpm <= 0 }) {
            Error(
                "PlaybackTempoMap: \"%\" needs a tempo in beats per minute above zero, got %."
                .format(text, bpm.asCompileString)
            ).throw
        };
        ^bpm
    }
