...
//
// Each step reads what the previous one wrote. Tempo is last because it
// wraps the clock instead of writing a note-event key.

// PlaybackProfile: the playback layers of a score, composed in one place.
//
// Four event layers, then tempo wrapping the finished pattern. The class owns
// that order and the carried-key clash check. The maps sit beside a score and
...
PlaybackProfile {
    ...
    *new { ^super.new }
    ...
    //
    // Refuse clashes here so layer order doesn't decide the sound.
    ...
    //
    // >>> PlaybackProfile.new.controlMap_(PlaybackControlMap.new.panAt(0, 0, 0)).copy.controlMap.controlsAt(0, 0)[\pan]
    // 0
    copy {
        ^PlaybackProfile.new
    ...
    // Apply the layer chain over one prepared tree.
    prEventsOf { |tree|
    ...
    // See Note [One key, one layer]. The message names both layers.
    prRefuseSharedKey { |clash|
        Error(
            "PlaybackProfile: % is written by both % and %. Give one layer a different "
            "key."
            .format(clash[0], clash[1], clash[2])
        ).throw
    }
    ...
    prChecked { |map, class, name|
        ...
        if (map.isKindOf(class).not) {
            Error(
                "PlaybackProfile: % takes %, got %."
                .format(name, class.name, map.class.name)
            ).throw
        };
        ^map
    }
