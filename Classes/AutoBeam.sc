// Note [Derived beams stay inside one metric context]
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
// A derived beam stays inside one bar, one voice and one beaming context, then
// stops at rests. Cross-barline and top-level-bracket beams stay authorial.
//
// A context is the outermost bracket around a leaf or its timeline. Inner
// brackets don't split a run inside the same outer bracket.
//
// One group to a beat. Grouped meters state beats directly. Ungrouped
// eighth-note meters beam in threes when divisible by three, otherwise in
// pairs.
//
// Mixed flag counts are allowed.
//
// See Note [A row per level] for the secondary beams and hooks.
//
...
AutoBeam {
    ...
    //
    // >>> AutoBeam.run(RhythmTree.measure(Meter(4, 8), [1, 1, 1, 1])).leaves.count { |leaf| leaf.hasSpanners }   -> 4
    *run { |element|
        ...
        Error(
            "AutoBeam.run: expected a MusicScore, Staff or Measure, got a %."
    ...
    *isBeamable { |leaf|
    ...
    //
    // The hook policy uses neighboring flag counts, not metric position.
    ...
    *rowsIn { |element|
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //
    // Every leaf must land in exactly one timeline and every endpoint
    // must pair in that timeline.
    //
    // If the partition loses source material or an endpoint can't pair, the
    // pass refuses instead of answering groups with missing beams. Writers call
    // this from `prepare`, so the writer-side refusal rule applies too.
    *groupsIn { |element|
        ^this.timelinesIn(element).inject([], { |all, leaves|
            all ++ this.prGroupsAlong(leaves) })
    }

    // Leaf timelines, in tree order. Staves use `Staff#timelines`. Bare bars
    // partition by voice. Fragments with no bar are one line. Otherwise the
    // partition has lost source material, so the beam pass refuses it.
    //
    ...
    *timelinesIn { |element|
    ...
    // Confirms the partition above kept every leaf.
    *prRequireEveryLeaf { |element, timelines|
        ...
        if (partitioned != total) {
            Error(
                "AutoBeam: % of this tree's % leaves are outside any timeline."
                .format(total - partitioned, total)
            ).throw
        };
        ^this
    }
    ...
    *prGroupsAlong { |leaves|
    ...
    // See Note [Pairing refuses what it cannot read].
    *prRefuseUnpairable { |leaf, starts, stops, run, openId|
        if (starts.notEmpty and: { stops.notEmpty }) {
            Error(
                "AutoBeam: % both stops and starts a beam. A note belongs to one beam "
                "group."
                .format(leaf)
            ).throw
        };
        if (starts.size > 1 or: { stops.size > 1 }) {
            Error(
                "AutoBeam: % carries % beam endpoints of one kind. Use at most one."
                .format(leaf, max(starts.size, stops.size))
            ).throw
        };
        ...
        if (stops.notEmpty and: { stops.first.id != openId }) {
            Error(
                "AutoBeam: beam ending on % has id %, but open beam id is %."
                .format(leaf, stops.first.id, openId)
            ).throw
        };
        if (starts.notEmpty and: { run.notNil }) {
            Error(
                "AutoBeam: beam begins on % while another is open."
                .format(leaf)
            ).throw
        };
        ^this
    ...
    *prBeamMeasure { |measure|
    ...
    //
    // The scan starts at the bar's own zero, matching `leafOffsetsIn`. Flag
    // changes stay in one group. Rows carry secondary beams.
    *prBeamRun { |leaves, spans, offsets|
        var run = [], context = nil, edge = Duration(0, 1), index = 0;
        var flush = { if (run.size > 1) { Spanner.beam(run) }; run = [] };

        // See Note [Derived beams stay inside one metric context]: a hand-beamed
        // voice is left alone.
    ...
    // The beaming context is the outermost bracket around the leaf or its
    // timeline.
