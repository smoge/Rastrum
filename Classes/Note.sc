// Leaves: a note, rest and chord.
//
// The names stay qualified because common SuperCollider setups
// already define `Note`, `Rest` or adjacent names.

// `tiesToNext` says this note continues into the next one. Ties are
// recorded on the earlier leaf. The model has no standalone stop.
//
...
MusicNote : ScoreLeaf {
    ...
    // A lone String is one note written out, `MN("c#8.")`. With a
    // separate duration, a pitch String fills a pitch slot:
    // `MN("c4")` is a C quarter and `MN("c[4]", "8")` an eighth in
    // octave 4. `text` and `placement` say in values what
    // `"c4:text{pizz.}"` says in a token. Written prose uses braces
    // because bare suffixes are closed vocabulary words.
    //
    ...
    *new { |pitch, dur, tiesToNext = false, text, placement = \above|
    ...
    // One token from the bar grammar. It goes through the same parser
    // so the two can't drift. Rests and chords are refused: this
    // builds a note.
    //
    ...
    *notation { |token, tiesToNext = false|
        ...
        if (token.isKindOf(String).not) {
            Error(
                "MusicNote.notation: expected a note String such as \"c4\" or \"c#8.\", got a %."
                .format(token.class)
            ).throw
        };
        ...
        if (ScoreNotation.prNotationTokens(token, "MusicNote.notation").size > 1) {
            Error(
                "MusicNote.notation: \"%\" is more than one leaf. Use Measure.notation for a bar."
                .format(token)
            ).throw
        };
        if (token.beginsWith("<")) {
            Error(
                "MusicNote.notation: \"%\" is a chord. Use Chord.notation."
                .format(token)
            ).throw
        };
    ...
    // Text helpers store as markings. Untied notes elide the flag.
    //
    // >>> MusicNote(MusicPitch("c[4]"), Duration(1, 4)).asCompileString
    // MusicNote(MusicPitch("c[4]"), Duration(1, 4))
    storeArgs { ^[pitch, dur, tiesToNext] }
}

// A shorthand for writing a note and nothing more. `MusicNote` is long
// in dense musical material. This buys the characters back. One name
// in a flat namespace is the whole cost.
MN {
...
MusicRest : ScoreLeaf {
    // A written duration may carry suffixes, with `r` implied.
    // A grace on a rest sounds before the silence.
    //
    ...
    *new { |dur, text, placement = \above|
        // A written rest names itself on a refusal. A Duration has no token.
        var where = if (dur.isKindOf(String)) {
            ScoreNotation.prLeafAt(dur, dur)
        } { nil };
    ...
    *prWrittenDuration { |dur|
    ...
    // >>> MusicRest(Duration(3, 8)).asCompileString   -> MusicRest(Duration(3, 8))
    storeArgs { ^[dur] }
}
...
Chord : ScoreLeaf {
    ...
    *new { |pitches, dur, tiesToNext = false, text, placement = \above|
    ...
    // `<c e g>4`, the pitches between the brackets and the length the
    // whole chord lasts.
    //
    // Pitches use `ScoreNotation.prNotationPitch`. The tail uses the
    // same length parser as note tokens.
    //
    // A repeated pitch is refused by `initChord`.
    //
    // >>> Chord("<c e g>4").pitches.size             -> 3
    ...
    *notation { |token, tiesToNext = false, label = "Chord.notation", where|
        ...
        if (token.isKindOf(String).not) {
            Error(
                "%: expected a chord String such as \"<c e g>4\", got a %."
                .format(label, token.class)
            ).throw
        };
        ...
        if (tied and: { tiesToNext.isKindOf(Boolean).not and: { tiesToNext.notNil } }) {
            Error(
                "%: % already ties the whole chord. Use either `~` or a per-pitch mask, not both."
                .format(label, where)
            ).throw
        };
        close = text.find(">");
        if (text.beginsWith("<").not or: { close.isNil }) {
            Error(
                "%: % is not a chord. Use pitches in angle brackets plus a length, e.g. \"<c e g>4\"."
                .format(label, where)
            ).throw
        };
        ...
        if (inside.isEmpty) {
            Error(
                "%: % has no chord pitches. Put at least one pitch inside the brackets."
                .format(label, where)
            ).throw
        };
        if (tail.isEmpty) {
            Error(
                "%: % has no chord length. Write one after the brackets."
                .format(label, where)
            ).throw
        };
        if (inside.includes($<) or: { tail.includes($>) }) {
            Error(
                "%: % nests its brackets. A chord is one leaf."
                .format(label, where)
            ).throw
        };
        ...
        if (inside.split($ ).any { |part|
            (part == "r") or: { part == "R" } }) {
            Error(
                "%: % puts a rest inside a chord. Use pitches only, or a MusicRest leaf."
                .format(label, where)
            ).throw
        };
        ...
        // A `~` on a pitch is that pitch's tie.
        // See Note [A tilde binds to what it follows] in ScoreNotationLeaves.sc.
        split = ScoreNotation.prChordTieMask(parts, where, label);
        written = split[1];
        if (written.notNil and: {
            tied or: { (tiesToNext ? false) != false } }) {
            Error(
                "%: % writes per-pitch ties and another tie setting. Use one or the other."
                .format(label, where)
            ).throw
        };
        built = this.new(
        ...
        ^ScoreNotation.prGraced(
            ScoreNotation.prMarked(built, marks[1], where), marks[2])
    }
    ...
    // A chord has at least one pitch and each pitch may appear once.
    // Repeats would merge tied runs.
    prCheckedPitches { |list|
        if (list.isEmpty) {
            Error(
                "Chord: a chord needs at least one pitch. Use a rest for silence."
            ).throw
        };
        list.do { |pitch, index|
            ...
            if (index > 0 and: { earlier.any { |other| other == pitch } }) {
                Error(
                    "Chord: % appears twice. Use separate voices for unison doublings."
                    .format(pitch)
                ).throw
            }
        };
    ...
    // True or false sets every pitch.
    //
    // One flag per pitch sets them apart.
    //
    // >>> Chord([60, 64, 67], Duration(1, 4)).tiesToNext      -> [ false, false, false ]
    ...
    prTieMask { |spec|
        ...
        if (spec.isSequenceableCollection.not) {
            Error(
                "Chord: tiesToNext must be true, false, or one Boolean per pitch, got a %."
                .format(spec.class)
            ).throw
        };
        if (spec.size != pitches.size) {
            Error(
                "Chord: % tie flags for % pitches. Use one flag per pitch."
                .format(spec.size, pitches.size)
            ).throw
        };
        ...
        spec.do { |flag, i|
            if (flag.isKindOf(Boolean).not) {
                Error(
                    "Chord: tie flag % must be Boolean, got % in %."
                    .format(i, flag.class, spec)
                ).throw
            }
        };
    ...
    //
    // >>> Chord([60, 64, 67], Duration(1, 4), [true, false, true]).tiedPitches.size   -> 2
    tiedPitches {
        var acc = List.new;
    ...
    // >>> Chord([60, 64, 67], Duration(1, 4), [true, false, true]).tiesAll -> false
    ...
    // Store the public tie argument: false, true or a partial mask.
    //
    // >>> Chord([60, 64], Duration(1, 4)).asCompileString
    // Chord([MusicPitch("c[4]"), MusicPitch("e[4]")], Duration(1, 4))
    // >>> Chord([60, 64], Duration(1, 4), true).asCompileString.contains("true")
    // true
    storeArgs {
        if (this.tiesAnything.not) { ^[pitches, dur] };
        ^[pitches, dur, if (this.tiesAll) { true } { tiesToNext }]
    }
}
