...
RhythmCell {
    ...
    *new { |proportions|
    ...
    //
    // A subdivided share is always `(weight (subshares))`. The second
    // element is a cell, not a bare share.
    //
    ...
    *prRtmList { |text, label|
        ...
        if (text.isKindOf(String).not) {
            Error(
                "%: expected RTM String like \"(1 (1 1) 2)\", got %."
        ...
        if (parsed[1] != tokens.size) {
            Error(
                "%: \"%\" has text after the closing paren."
                .format(label, text)
            ).throw
        };
        ^parsed[0]
    ...
    *prRtmCell { |tokens, at, whole, label|
        ...
        if (tokens[at] != "(") {
            Error(
                "%: \"%\" needs a cell where % stands. Use parens around shares."
                .format(
                    label,
                    whole,
                    this.prRtmAt(tokens, at)
                )
            ).throw
        };
        index = index + 1;
    ...
    *prRtmShare { |tokens, at, whole, label|
        ...
        if (tokens[inner[1]] != ")") {
            Error(
                "%: \"%\" leaves a subdivided share unclosed. Use (weight (subshares))."
                .format(label, whole)
            ).throw
        };
    ...
    *prRtmWeight { |token, whole, label|
        ...
        if (token.isNil or: { token == "(" } or: { token == ")" }) {
            Error(
                "%: \"%\" needs an integer share where % stands."
                .format(
                    label,
                    whole,
                    token ?? { "the end of the text" }
                )
            ).throw
        };
    ...
    *prRtmAt { |tokens, at| ^(tokens[at] ?? { "the end of the text" }) }
    ...
    //
    // >>> { var c = RhythmCell([1, [2, [3, 4]]]); var p = c.proportions; p[1][1][0] = 99; c.proportions }.value
    // [ 1, [ 2, [ 3, 4 ] ] ]
    ...
    *asProportions { |value|
    ...
    // Whether this is the written list form of one cell.
    //
    // Strings and `RhythmCell` instances are cells by class, not by this test.
    //
    // >>> RhythmCell.isShareList([1, 1])             -> true
    // >>> RhythmCell.isShareList([[1, 1]])           -> false
    // >>> RhythmCell.isShareList([3, [2, [1, 1]]])   -> true
    // >>> RhythmCell.isShareList("(1 1)")            -> false
    *isShareList { |value|
        ...
        ^value.every { |entry| this.isShare(entry) }
    }

    // A share is a nonzero whole weight or a weight over a share list.
    //
    // >>> RhythmCell.isShare(-2)           -> true
    // >>> RhythmCell.isShare(0)            -> false
    // >>> RhythmCell.isShare([1, [1, 1]])  -> true
    // >>> RhythmCell.isShare([1, 1])       -> false
    *isShare { |entry|
        if (entry.isKindOf(Integer)) { ^entry != 0 };
        if (entry.isSequenceableCollection.not or: { entry.isKindOf(String) }) {
            ^false
        };
        if (entry.size != 2) { ^false };
        ^entry[0].isKindOf(Integer) and: { entry[0] != 0 }
            and: { this.isShareList(entry[1]) }
    }
    ...
    *checkedProportions { |list|
        ...
        if (entries.isSequenceableCollection.not or: { entries.isKindOf(String) }) {
            Error(
                "RhythmCell: % is not a proportion list. Use shares or [weight, subdivisions]."
                .format(list)
            ).throw
        };
        if (entries.isEmpty) {
    ...
    *checkedEntry { |entry|
        ...
        if (entry.isSequenceableCollection.not or: { entry.isKindOf(String) }) {
            Error(
                "RhythmCell: % is not a share. Use an integer or [weight, subdivisions]."
                .format(entry)
            ).throw
        };
        if (entry.size != 2) {
            Error(
                "RhythmCell: subdivided share % must be [weight, subdivisions]."
                .format(entry)
            ).throw
        };
    ...
    *checkedWeight { |value|
    ...
    //
    // One bar. Direct material starts here. A `PitchStream` is shared
    //
    // See Note [One call, one row] in RhythmTree.sc.
    //
    ...
    scaledBy { |factor|
        if (factor.isKindOf(Integer).not or: { factor < 1 }) {
            Error(
                "RhythmCell: scale factor must be a positive Integer, got %."
                .format(factor)
            ).throw
        };
    ...
    *prScaled { |list, factor|
    ...
    // Answers the cell backwards, subdivisions included.
    //
    // Recursive: the sounding rhythm read end to end is reversed.
    //
    ...
    *prReversed { |list|
    ...
    //
    // Paths, not cells: a cell can't say where it was.
    //
    ...
    *prCellPaths { |list, prefix|
    ...
    subdivideAt { |path, cell|
        ...
        if (share.isNumber.not) {
            Error(
                "RhythmCell: share at % is already subdivided. Use replaceCellAt."
                .format(path)
            ).throw
        };
        if (share < 0) {
            Error(
                "RhythmCell: the share at % is a rest. Use replaceAt to build a "
                "negative subdivided head explicitly."
                .format(path)
            ).throw
        };
    ...
    // Replaces one share's subdivisions, keeping its weight and sign.
    // `replaceAt` replaces the whole share.
    //
    // This is the inside-only form.
    //
    ...
    prSubdividedShareAt { |path|
        ...
        if (share.isNumber) {
            Error(
                "RhythmCell: the share at % is not subdivided. It is %."
                .format(path, share)
            ).throw
        };
        ^share
    ...
    *prReplaced { |list, steps, share|
    ...
    // Silences shares at paths. A subdivided share becomes one rest
    // of its weight, as in `mutedAt`.
    //
    ...
    // A path may not sit inside another path. Silencing both would
        ...
        paths.do { |outer|
            paths.do { |inner|
                if (inner.size > outer.size and: {
                    ...
                    Error(
                        "RhythmCell: path % is inside path %. Silence one or the other."
                        .format(inner, outer)
                    ).throw
                }
            }
    ...
    *prOnePath { |path|
        ...
        if (path.isSequenceableCollection.not or: { path.isKindOf(String) }) {
            Error(
                "RhythmCell: % is not a path. Use an index or index Array."
                .format(path)
            ).throw
        };
        steps = path.asArray;
        ...
        if (steps.every { |step| step.isSequenceableCollection }) {
            Error(
                "RhythmCell: % names multiple paths. Use mutedAtPath."
                .format(path)
            ).throw
        };
        steps.do { |step|
            if (step.isNumber.not) {
                Error(
                    "RhythmCell: path step % must be an Integer of zero or more."
                    .format(step)
                ).throw
            }
        };
    ...
    *prPathList { |paths|
        ...
        Error(
            "RhythmCell: % mixes path steps and paths. Use one path or a list of paths."
            .format(paths)
        ).throw
    }
    ...
    prCheckedPath { |path|
        ...
        steps.do { |step, depth|
            ...
            if (step < 0 or: { step >= list.size }) {
                Error(
                    "RhythmCell: path % has no share at position %. This level has % shares."
                    .format(steps, step, list.size)
                ).throw
            };
            if (depth < (steps.size - 1)) {
                ...
                if (entry.isNumber) {
                    Error(
                        "RhythmCell: the path % descends into the share at position %, "
                        "which is the plain share % and has nothing inside it."
                        .format(steps, step, entry)
                    ).throw
                };
                list = entry[1];
            }
    ...
    prCheckedIndex { |index|
        if (index.isKindOf(Integer).not) {
            Error(
                "RhythmCell: position must be a non-negative Integer, got %."
                .format(index)
            ).throw
        };
        if (index < 0 or: { index >= this.size }) {
            Error(
                "RhythmCell: no share at position %. This cell has % shares."
                .format(index, this.size)
            ).throw
        };
        ^index
    }
    ...
    // `RhythmTree.fill`, so nothing here reduces weights or knows
    // tuplets.
    //
    // One row per terminal share: `path`, `offset`, `duration`,
    // `weight`, `sounds`.
    ...
    *prWeightOf { |entry| ^if (entry.isNumber) { entry.abs } { entry[0].abs } }
    ...
    // Terminal shares including rests. Span-free: a count being a count
    //
    ...
    *prDepth { |list|
    ...
    // This cell with common factors removed independently at each
    // sibling list, the same way lowering reads it.
    //
    // `==` is structural. `[1, 1]` and `[2, 2]` are different cells,
    // even when they write the same rhythm. `reduced` keeps nesting
    // and only divides local sibling weights.
    //
    // `sameTimingAs` and `sameAttacksAs` ask the rhythm question.
    //
    // Per level and independently, as the lowering does it.
    //
    // >>> RhythmCell([2, [4, [2, 2]], -6]).reduced
    // RhythmCell([ 1, [ 2, [ 1, 1 ] ], -3 ])
    reduced { ^RhythmCell(RhythmCell.prReduced(proportions)) }

    // `Rational.gcd`, not sclang's `gcd:`, which answers -2 for
    ...
    *prReducedEntry { |entry, factor|
    ...
    // Where the attacks fall in a span. Usually that is the question
    // a comparison between two cells asks.
    //
    ...
    // flat cell. Neither is scaling. Silence is, a rest being a share
    // of the span.
    //
    ...
    //
    // The weaker question, often the one being asked: where a
    ...
    *distinct { |cells, mode, span|
        ...
        if (selector.isNil) {
            Error(
                "RhythmCell.distinct: mode must be timing or attacks, got %."
                .format(mode)
            ).throw
        };
        if (cells.isSequenceableCollection.not or: { cells.isKindOf(String) }) {
            Error(
                "RhythmCell.distinct: expected an Array of cells or proportion lists, got %."
                .format(cells)
            ).throw
        };
        cells.do { |cell|
            // Existing cells keep live identity, so callers can find them
            // in the source pool with `includes`.
    ...
    *prModeKeys { ^(timing: \prTimingIn, attacks: \attackOffsetsIn) }
    ...
    // order is preserved and nothing is reduced, filtered or measured
    // against a span. That is caller policy. Silence is added later
    // with `mutedAt`, since a negative share would make the sum mean
    // two things.
    //
    ...
    *fieldFor { |meter, weights, counts, mode|
        ...
        if (span.isNil) {
            Error(
                "RhythmCell.fieldFor: expected a Meter or time signature, got nil."
            ).throw
        };
    ...
    *prCheckedWeights { |weights|
        ...
        pool.do { |each|
            if (each < 1) {
                Error(
                    "RhythmCell.allFrom: weight must be positive, got %."
                    .format(each)
                ).throw
            }
        };
    ...
    // every cell again. Refusing all three is one contract rather
    // than a rule with an exception.
    *prIntegerList { |value, what, least|
        var list;
        if (value.isKindOf(String)) {
            Error(
                "RhythmCell.allFrom: % must be whole numbers, got String %."
        ...
        list.do { |each, index|
            if (each.isKindOf(Integer).not) {
                Error(
                    "RhythmCell.allFrom: % must contain Integers, got %."
                    .format(what, each.asCompileString)
                ).throw
            };
            if (least.notNil and: { each < least }) {
                Error(
                    "RhythmCell.allFrom: % must be % or more, got %."
                    .format(what, least, each.asCompileString)
                ).throw
            };
            if (list.indexOf(each) != index) {
                Error(
                    "RhythmCell.allFrom: % contain duplicate value %."
                    .format(what, each)
                ).throw
            }
        };
    ...
    // Structural equality. Timing questions use `sameTimingAs` or
    // `sameAttacksAs`.
    //
    ...
    // Non-empty proportions never elide to `RhythmCell.new`.
    //
    // >>> RhythmCell([1, [2, [1, 1]]]).asCompileString
    // RhythmCell([1, [2, [1, 1]]])
    storeArgs { ^[proportions] }
}
